...
class SuperMaruetsu(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/maruetsu'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        areas = soup.find('div', {'id': 'storeListBox'}).find_all('ul', {'class': 'storeList'})
        for area in areas:
...
_store['address'] = details[0].text.replace('\n', '').replace('\r', '')
            ...
            raise TypeError('Store has been closed.')
        maps = soup.find_all('iframe')[1]['src']
