...
class SweetsBoulmich(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/sweets/boulmich'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        areas = soup.find_all('table', 'storeTable')
        for area in areas:
            ...
            for store in store_lists:
                ...
                _store['store_name'] = details[0].text.split('【')[0].replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', '').replace(' ', '')
