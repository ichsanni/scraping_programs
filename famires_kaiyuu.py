...
class FamiresKaiyuu(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_famires_kaiyuu.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        store_lists = soup.find_all('p', 'btn-shop')
    ...
    def get_data(self, url):
        ...
        soup = BeautifulSoup(page.html.html, 'html.parser')
        _store = dict()
        try: _store['store_name'] = soup.find('h1').find('img')['alt'].split('蟹遊亭(かいゆうてい)')[1].strip()
        except IndexError: _store['store_name'] = soup.find('h1').find('img')['alt'].split('蟹遊亭')[1].strip()
_store['address'] = soup.find('dt', text='住所').find_next_sibling('dd').text
        ...
        _store['gla'] = ''
        _store['定休日'] = re.sub('\s+', ' ', soup.find('dt', text='休業日').find_next_sibling('dd').text) # Regular holiday
        _store['駐車場'] = re.sub('\s+', ' ', soup.find('dt', text='駐車場').find_next_sibling('dd').text) # Parking lot Yes ( 有 ) , No ( 無 )
