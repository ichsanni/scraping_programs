...
class ZakkaThankyoumart(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/zakka/thankyoumart'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        store_lists = soup.find_all('a', 'toShopDetail')
  ...
  """
        ...
        _store['store_name'] = soup.find('div', 'shopName').text.strip()
_store['address'] = soup.find('p', text='住所').find_next_sibling('p').text.strip().replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', ' ')

        _store['lat'], _store['lon'] = '', ''
        _store['tel_no'] = ''
        _store['open_hours'] = soup.find('p', text='営業時間').find_next_sibling('p').text.strip().replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', ' ')
        _store['gla'] = ''
