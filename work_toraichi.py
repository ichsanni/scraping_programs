...
class WorkToraichi(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/work/toraichi'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find_all('div', 'prefectures-details-inner')
        print(len(store_lists))
        for store in store_lists:
            _store = dict()
            _store['url_store'] = url
            _store['store_name'] = store.find('h4').text
_store['address'] = store.find('dt', text='所在地').find_next_sibling('dd').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', ' ').replace('MAP', '')
            # maps_link = soup.find('dl').find_next_sibling('p').find('a')['href']
...
_store['address']
            ...
            _store['tel_no'] = store.find('dt', text='電話番号').find_next_sibling('dd').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', ' ')
            _store['open_hours'] = store.find('dt', text='営業時間').find_next_sibling('dd').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', ' ')
            _store['gla'] = ''
