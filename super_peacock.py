...
class SuperPeacock(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/peacock'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        domain = "REDACTED URL"
        store_links = soup.find_all('p', {'class': 'place'})
  ...
  """
        ...
        soup = BeautifulSoup(page.html.html, 'html.parser')
        _store = dict()
        details = soup.find_all('td', {'class': 'description'})
        _store['url_store'] = url
        _store['store_name'] = soup.find('h1').text
_store['address'] = details[0].text.replace('\xa0', '')
