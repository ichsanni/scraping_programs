...
class PetGojohpet(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/pet/gojohpet'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find_all('p', 'idx_btn_tit')
...
_store['address'] = soup.find('th', text='住所').find_next_sibling('td').text
        ...
        _store['tel_no'] = soup.find('th', text='連絡先').find_next_sibling('td').text.replace('TEL', '').replace('FAX：', '').replace('：', '').replace('/', '')
