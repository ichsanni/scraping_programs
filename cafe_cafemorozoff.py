...
class CafeCafemorozoff(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/cafe/cafemorozoff'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        areas = soup.find('section', 'first').find_all('a')
        for area in areas:
            ...
            store_lists = area_soup.find_all('li', 'cafe')[1:]
            for store in store_lists:
                store = store.parent.parent.parent.parent
                _store = dict()
                _store['url_store'] = url
                _store['store_name'] = store.find('h3').text.replace('\u3000', ' ')
_store['address'] = store.find('dt', text='住所').find_next_sibling('dd').text.replace('Google MAP', '').replace('\n', '').strip()
