...
class SuperDaiyusuper(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/daiyusuper'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        for store in store_lists:
            if '店' in store.find('h2').text:
                details = store.find('p', 'store_note').text
                _store = dict()
                _store['url_store'] = url
                _store['store_name'] = store.find('h2').text
_store['address'] = re.findall('〒.+\\s*.+T', details)[0].replace('T', '').replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', '').replace(' ', '')

                _store['lat'], _store['lon'] = '', ''
                _store['tel_no'] = re.findall('TEL：\\d+-\\d+-\\d+', details)[0].replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', '').replace('TEL：', '')
                _store['open_hours'] = re.findall('営業時間.+\\n', details)[0].replace('\u2003', '').replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', '').replace('営業時間', '')
                _store['gla'] = ''
