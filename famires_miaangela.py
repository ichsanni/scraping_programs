...
class FamiresMiaangela(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_famires_miaangela.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        store_lists = soup.find_all('article', 'detail')
        for store in store_lists:
            _store = dict()
            _store['store_name'] = store.find('h4').text.replace(store.find('h4').find('span').text, '')
_store['address'] = store.find('dt', text='住所').find_next_sibling('dd').text
            ...
            _store['駐車場'] = store.find('dt', text='駐車場').find_next_sibling('dd').text # Parking lot Yes ( 有 ) , No ( 無 )
            _store['禁煙・喫煙'] = '' # [Non-smoking/Smoking] Yes ( 有 ) , No ( 無 )
            _store['取扱'] = '' # Handling
            _store['備考'] = '[席数] ' + store.find('dt', text='席数').find_next_sibling('dd').text # Remarks
