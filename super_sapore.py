...
class SuperSapore(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/sapore'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        for store in store_lists:
            if store['href'] not in self.visited:
                self.visited.append(store['href'])
  ...
  """
        ...
        if store_name != '':
            ...
            _store['store_name'] = soup.find_all('h2')[1].text
_store['address'] = soup.find('span', text='住所').parent.text.replace('住所', '').replace('\u3000', '')

        _store['lat'], _store['lon'] = '', ''
        _store['tel_no'] = soup.find('span', text='TEL').parent.text.replace('TEL', '')
