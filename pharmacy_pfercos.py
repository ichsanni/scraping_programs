...
class PharmacyPfercos(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/pharmacy/pfercos'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        store_lists = soup.find_all('a', 'result-box')
        for store in store_lists:
            ...
            time.sleep(0.5)
        try:
            next_button = soup.find('a', 'next')['href']
...
_store['address'] = soup.find('dt', text='住所：').find_next_sibling('dd').text.replace('\n', '').replace(' ', '').replace('\u3000', '').replace('\r', '')
        ...
        try: _store['tel_no'] = soup.find('dt', text='TEL：').find_next_sibling('dd').text.replace('\n', '').replace(' ', '').replace('\u3000', '').replace('\r', '')
