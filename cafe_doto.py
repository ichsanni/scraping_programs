...
class CafeDoto(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/cafe/doto'.replace("/", "_")
        self.content = list()
        ...
        for pref in home_json['data']:
            ...
            for area in area_json['data']:
                ...
                for point in point_json['items']:
                    if 'ドトール' in point['name'] and point['key'] not in self.pg_code:
                        link = domain + point['key']
                        data = loop.run_until_complete(self.get_data(link, point['latitude'], point['longitude']))
                        if data:
