...
class Riyobiyo11cut(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/riyobiyo/11cut'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        areas = soup.find_all('table', 'tb_style')
        for area in areas:
            store_lists = area.find_all('tr')
            for store in store_lists:
                store_url = "REDACTED URL"+ store.find('a')['href'].replace('./', '')
  ...
  """
        ...
        _store['store_name'] = soup.find('h4').find('span').text
_store['address'] = soup.find('th', text='住所：').find_next_sibling('td').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', ' ')
        ...
        _store['open_hours'] = soup.find('th', text='受付時間：').find_next_sibling('td').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', ' ')
        _store['gla'] = ''
