...
class HomecenterWorkway(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/homecenter/workway'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.select('li.uketori > a')
...
_store['address'] = soup.find('dt', text=re.compile('住所')).find_next_sibling('dd').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', '').replace(' ', '')
        # iframe_maps = soup.find('div', {'id':'shopMap'}).find('iframe')['data-src']
...
_store['address']
        ...
        _store['tel_no'] = soup.find('dt', text=re.compile('電話番号')).find_next_sibling('dd').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', '').replace(' ', '')
