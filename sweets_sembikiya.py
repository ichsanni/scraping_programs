...
class SweetsSembikiya(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_sweets_sembikiya.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        store_lists = soup.select("h2 a[href*='shop-list']")
        for store in store_lists:
            link = store['href']
            store = store.parent.parent.parent.parent

            _store = dict()
            _store['store_name'] = store.find('h2').texttry:
                maps_link = store.select_one("p a[href*='/maps/']")
                
_store['address'] = maps_link.parent.text.replace(maps_link.text, '')
            except AttributeError:
                maps_link = store.select_one("a[href*='/maps/']")
                
_store['address'] = maps_link.find_previous_sibling('p').text.replace(maps_link.text, '')
            _store['url_store'] = f"REDACTED URL"
            _store['url_tenant'] = ''
            try: _store['営業時間'] = store.find('h5', text=re.compile('営業時間')).find_next_sibling('p').text # Open hours / Business Hours
            except AttributeError: _store['営業時間'] = ''
            coords = self.geo_loc_embed(maps_link['href'])
            _store['lat'] = coords[0]
            _store['lon'] = coords[1]
            _store['tel_no'] = store.find('a', 'tel').text
            _store['gla'] = ''
            try: _store['定休日'] = store.find('h5', text=re.compile('定休日')).find_next_sibling('p').text # Regular holiday
