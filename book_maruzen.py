...
class BookMaruzen(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/book/maruzen'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        areas = soup.find_all('table', {'class': 'stTableData01'})
        for area in areas:
            store_lists = area.find_all('tr')
            for store in store_lists:
                data = await self.get_data(store.find('a')['href'])
                if data != '':
                    self.content.append(data)
                    self.save_data()
                    print(len(self.content))
        try:
            next_button = soup.find('li', 'stNext').find('a', text='次へ')['href']
...
_store['address'] = soup.find('th', text='住所').find_next('td').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', '')
        maps = soup.find('th', text='店舗地図').find_next('td').find('a')['href']

       
        # print(lat)

        _store['lat'], _store['lon'] = '', ''
        _store['tel_no'] = soup.find('th', text='電話番号').find_next('td').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', '')
