...
class HomecenterHomecentervalor(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/homecenter/homecentervalor'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        for store in store_lists:
            try:
                link = store.find_all('td')[0].find('a').get('href')
  ...
  """
        ...
        _store['store_name'] = soup.find('h1').textinfo = soup.find('p', {'class': 'info'}).text.split('：')
        
_store['address'] = info[0].replace('TEL', '').replace('\r', '').replace('\n', '').replace('\u3000', '')

        _store['lat'], _store['lon'] = '', ''

        _store['tel_no'] = info[1].replace('-', '').replace('FAX', '').replace('\r', '').replace('\n', '').replace('\u3000', '')
        _store['open_hours'] = soup.find('div', {'class': 'box'}).find('p').text.replace('\r', '').replace('\n', '').replace('\u3000', '')
        _store['gla'] = 'Null'
...
if __name__ == '__main__':
    HomecenterHomecentervalor(True)
