...
class SuperThelows(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/thelows'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find('div', {'id': 'wrapper'}).find_all('a')
        for store in store_lists:
            if '../' not in store['href']:
  ...
  """
        ...
        _store['store_name'] = soup.find('th', text=re.compile('店名')).find_next_sibling('td').text.replace('\n', '').replace('\xa0', '').strip()
_store['address'] = soup.find('th', text=re.compile('所在地')).find_next_sibling('td').text.replace('\n', '').replace('\xa0', '').strip()

        _store['lat'], _store['lon'] = '', ''
        _store['tel_no'] = soup.find('th', text=re.compile('電話')).find_next_sibling('td').text.replace('\n', '').replace('\xa0', '').strip()
