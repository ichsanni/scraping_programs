...
class BankTokyostarbank(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/bank/tokyostarbank'.replace("/", "_")
        self.content = list()
...
_store['address']
            ...
            _store['tel_no'] = ''
            try:
                open_hours = [x['value'] for x in store['details'][0]['texts'] if x['label'] == '営業時間\u3000ATM'][0].lower().replace('br', '').replace('<', '').replace('>', '')
