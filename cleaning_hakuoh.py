...
class CleaningHakuoh(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/cleaning/hakuoh'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find_all('td', 'tenmei')
...
_store['address'] = soup.find('th', text=re.compile('住\\s+所')).find_next_sibling('td').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', ' ')

        _store['lat'], _store['lon'] = '', ''
        try:
            _store['tel_no'] = soup.find('th', text=re.compile('電\\s+話')).find_next_sibling('td').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', ' ')
