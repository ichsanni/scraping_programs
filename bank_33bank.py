...
class Bank33bank(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_bank_33bank.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self):
        ...
        while total >= offset:
            params = {
                ...
                'limit': limit,
                'offset': offset,
                'timeStamp': '202303060956',
                'timeStamp': datetime.datetime.now().strftime('%Y%m%d%H%M')
            }
            offset += limit
            store_lists = self.session.get("REDACTED URL" params=params,
...
_store['address'] = '〒' + store['postal_code'][:3] +'-'+ store['postal_code'][3:] + store['address_name']
                ...
                _store['url_tenant'] = ''
                _store['営業時間'] = ''.join([x['value'] for x in store['details'][0]['texts'] if x['label'] == '営業時間'])  # Open hours / Business Hours
                ...
                _store['取扱'] = '' # Handling
                _store['備考'] = '[ATM]' + ''.join([x['value'] for x in store['details'][0]['texts'] if x['label'] == 'ATM利用時間']).replace('<br>', ' ')  # Remarks
