...
class JukuSapporoshingaku(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_juku_sapporoshingaku.py'.replace('/', '_').replace('.py', '')
        self.from_main = from_main
        self.session = HTMLSession()
        self.content = list()

        start = time.time()
        # NOTE: https://www.sapporo-plaza.jp/
        # klik salah satu tombol di bagian search
        url = "REDACTED URL"
        self.get_page(url)
        end = time.time()
    ...
    def get_page(self, url):
        ...
        data = {
            's_type[]': '集団指導',     # filter berdasarkan "group instruction" only
        }

        page = self.session.post(url, headers=headers, data=data)
        page = json.loads(page.html.html.strip('();'))['html']
        soup = BeautifulSoup(page, 'html.parser')

        store_lists = soup.find_all('div', 'result')
        for store in store_lists:
            _store = dict()
            _store['store_name'] = store.find('p', 'name').text.strip()
_store['address'] = store.find('p', 'address').text.strip()
            _store['url_store'] = url
            _store['url_tenant'] = ''
            _store['営業時間'] = '' # Open hours / Business Hours
            maps_link = store.find('a', text='大きい地図はこちら')['href']
            coords = self.geo_loc_embed(maps_link)
            _store['lat'] = coords[0]
            _store['lon'] = coords[1]
            _store['tel_no'] = store.find('p', 'tel').text.strip()
            _store['gla'] = ''
