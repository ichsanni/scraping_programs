...
class KaitenzushiUobei(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/kaitenzushi/uobei'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find_all('img', {'src': 'img/icon/uobei.png'})
        for store in store_lists:
            link = "REDACTED URL"+ store.find_previous('a')['href']
  ...
  """
        ...
        _store['store_name'] = soup.find('dd').text
_store['address'] = soup.find('th', text='■住所').find_next('td').text.replace('\xa0', ' ')
...
_store['address']
        ...
        _store['lat'] = ''
        _store['lon'] = ''
        _store['tel_no'] = soup.find('th', text='■電話番号').find_next('td').text
        try: _store['open_hours'] = soup.find('th', text='■営業時間(平日)').find_next('td').text + '(平日) ' + soup.find('th', text='■営業時間(土日祝)').find_next('td').text + '(土日祝)'
