...
class CdHmv(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/cd/hmv'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find('div', 'stMainCol').find_all('a')
        for store in store_lists:
            if '/store/' in store['href'] and '#' not in store['href'] and ('hatena' not in store['href']):
...
_store['address'] = soup.find('th', text='住所：').find_next_sibling('td').text.replace('\u3000', ' ').replace(',', '-')
        maps_link = soup.find('div', {'id': 'googlemap'}).find('iframe')['src']
