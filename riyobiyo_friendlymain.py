...
class RiyobiyoFriendlymain(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/riyobiyo/friendlymain'.replace("/", "_")
        self.content = list()
...
_store['address'] = soup.find('td', text=re.compile('郵便番号')).find_next_sibling('td').text.strip() + ' ' + soup.find('td', text=re.compile('住所')).find_next_sibling('td').text.strip().replace('\u3000', ' ')

        _store['lat'], _store['lon'] = '', ''
        _store['tel_no'] = soup.find('td', text=re.compile('ＴＥＬ')).find_next_sibling('td').text.strip()
        _store['open_hours'] = soup.find('td', text=re.compile('営業時間')).find_next_sibling('td').text.strip()
        _store['gla'] = ''
