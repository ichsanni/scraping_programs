...
class PharmacyPhk(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/pharmacy/phk'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find_all('div', 'pos_rel')
  ...
  """
        ...
        _store['store_name'] = soup.find('h2', 'pagettl').text.replace(' ', '').replace('\u3000', '')
_store['address'] = soup.find('th', text='住所').find_next_sibling('td').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', '')
        try:
            maps = soup.find('a', text='Google mapで拡大地図を見る')['href']
        except TypeError:
