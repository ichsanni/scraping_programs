...
class RamenRengetoshu(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/ramen/rengetoshu'.replace("/", "_")
        self.content = list()
...
_store['address']
            ...
            try:
                ...
                _store['tel_no'] = ''
            try:
                open_hours = [x['value'] for x in store['details'][0]['texts'] if x['label'] == '営業時間'][0].lower().replace('br', '').replace('<', '').replace('>', '')
