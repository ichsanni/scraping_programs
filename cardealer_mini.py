...
class CardealerMini(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/cardealer/mini'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        page =self.session.get(url, headers=headers).html.html.replace('angular.callbacks._1(', '').replace('})', '}')
        store_lists = json.loads(page)
        content = list()
        for store in store_lists['data']['pois']:
            _store = dict()
            if not store['attributes']['homepage'] or store['attributes']['homepage'] == '':
                continue
