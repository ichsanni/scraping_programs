...
class YakinikuShoutaian(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_yakiniku_shoutaian.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        store_lists = soup.select('article > h3')
        for store in store_lists:
            store = store.parent        # balik ke tag article
    ...
    def get_data(self, url):
        ...
        page = self.session.get(url, headers=headers)
        soup = BeautifulSoup(page.html.html, 'html.parser')
        details = soup.find_all('h2', 'top_tit')[1].parent
        _store = dict()
        _store['store_name'] = details.find('h3').text
_store['address'] = details.find('address').text.split("TEL")[0]
        _store['url_store'] = url
        _store['url_tenant'] = ''
        try: _store['営業時間'] = soup.find('dt', text='dinner').find_next_sibling('dd').text # Open hours / Business Hours
        except AttributeError:
            try: _store['営業時間'] = soup.find('dt', text='土日祝').find_next_sibling('dd').text
            except AttributeError: _store['営業時間'] = soup.find('dt', text='open').find_next_sibling('dd').text
        maps_link = soup.select_one('a[href*="/maps/"]')['href']
        coords = self.geo_loc_embed(maps_link)
        _store['lat'] = coords[0]
        _store['lon'] = coords[1]
        _store['tel_no'] = details.find('address').text.split("TEL")[1].replace(':', '').strip()
        _store['gla'] = ''
