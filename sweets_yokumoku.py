...
class SweetsYokumoku(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/sweets/yokumoku'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        areas = soup.find('ul', 'map').find_all('li')
        for area in areas:
            area_page =self.session.get(area.find('a')['href'], headers=headers)
            area_soup = BeautifulSoup(area_page.html.html, 'html.parser')
            store_lists = area_soup.find_all('div', 'shop_name')
  ...
  """
        print(url)
        if '/usa/' in url or '/others/' in url:
            return
        headers = {'user-agent': UserAgent().random.strip()}
        page =self.session.get(url, headers=headers)
        soup = BeautifulSoup(page.html.html, 'html.parser')
        ...
        _store['store_name'] = soup.find('div', 'tit_sty_ja').text
...
_store['address']
        ...
        _store['tel_no'] = soup.find('div', text='TEL').find_next_sibling('div').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('-', '')
        try:
