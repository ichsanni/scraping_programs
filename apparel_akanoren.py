...
class ApparelAkanoren(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/apparel/akanoren'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find_all('h3')[1:]
        for store in store_lists:
            details = store.parent
            _store = dict()
            _store['url_store'] = url
            _store['store_name'] = store.text
_store['address'] = details.find('p').text
            try:
...
_store['address']
            ...
            try:
                _store['tel_no'] = details.find('dt', text='［電話番号］').find_next('dd').text.replace('\n', '').replace('\t', '').replace('\r', '').replace(' ', '')
            except AttributeError:
                _store['tel_no'] = ''
            try:
                _store['open_hours'] = details.find('dt', text='［営業時間］').find_next('dd').text.replace('\n', '').replace('\t', '').replace('\r', '').replace(' ', '')
            except AttributeError:
                _store['open_hours'] = ''
            if _store['tel_no'] == '' and _store['open_hours'] == '':
                continue
            _store['gla'] = ''
