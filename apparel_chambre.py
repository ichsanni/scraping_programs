...
class ApparelChambre(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
        self.session = HTMLSession()
        self.file_name = '/apparel/chambre'.replace("/", "_")
        self.content = list()
        self.headers = ({'user-agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.79 Safari/537.36'}, {'user-agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_9_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.65 Safari/537.36'}, {'user-agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/77.0.3844.0 Safari/537.36'}, {'user-agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 11_1_0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4356.6 Safari/537.36'})
    ...
    def getdata(self, step=0):
        session = HTMLSession()
        req =self.session.get(self.url + 'step=' + str(step), headers=self.headers[0])
        print(req.url)
        ...
        for card in cards:
            ...
            try:
                ...
                continue
            req_store =self.session.get(url_store, headers=self.headers[1])
            ...
            # df.to_csv('/mnt/c/dags/hasil/_apparel_chambre.csv', index=False)
