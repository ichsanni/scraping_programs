...
class ChainIdToko(CleaningData):
  def __init__(self, from_main=False):
    self.file_name = '_juku_tanaka.py'.replace('/', '_').replace('.py', '')
  ...
  def get_page(self, url):
    ...
    for store in store_lists:
      if '田中' not in store.find('a').text: continue
...
_store['address'] = re.sub('\s+', ' ', soup.find('dt', text='住所').find_next_sibling('dd').text).strip()
...
_store['address']
    ...
    _store['open_hours'] = soup.find('dt', text='受付時間').find_next_sibling('dd').text.replace('\n' ,' ')
