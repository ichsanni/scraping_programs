...
class SuperMandai(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/mandai'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        domain = "REDACTED URL"
        areas = soup.find('div', {'class': 'arealist'}).find_all('ul', {'class': 'list'})
        for area in areas:
  ...
  """
        ...
        page =self.session.get(url, headers=headers)
        soup = BeautifulSoup(page.html.html, 'html.parser')
        store_lists = soup.find('ul', 'shoplist').find_all('li', recursive=False)
        for store in store_lists:
            _store = dict()
            _store['url_store'] = url
            _store['store_name'] = store.find('li', 'name').texttry:
                
_store['address'] = store.find('li', 'adrs').text
...
_store['address']
            ...
            _store['tel_no'] = store.find('li', 'tel').text
            _store['open_hours'] = store.find('li', 'hour').text
            _store['gla'] = ''
