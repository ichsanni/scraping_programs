...
class BuffetGrandbuffet(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/buffet/grandbuffet'.replace("/", "_")
        self.content = list()
...
_store['address'] = soup.find('div', 'text-4', text='住所').find_next_sibling('div').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', ' ').replace('ルートを検索', ' ').strip()
        ...
        _store['tel_no'] = soup.find('div', 'text-4', text='営業時間').find_next_sibling('div').find('div').text.replace('\n', ' ').replace('\t', '').replace('\r', ' ').replace('\u3000', ' ').strip()
        _store['open_hours'] = soup.find('div', 'text-4', text='電話番号・予約').find_next_sibling('div').find('a').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', ' ').strip()
        _store['gla'] = ''
