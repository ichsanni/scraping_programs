...
class BankTono(CleaningData):
  def __init__(self, from_main=False):
    self.file_name = '_bank_tono.py'.replace('/', '_').replace('.py', '')
    ...
    start = time.time()
    # 35.270674699055554, 136.95564194161327
    url = "REDACTED URL"
    self.get_page(url)
    end = time.time()
  ...
  def get_page(self, url):
    ...
    for store in store_lists:
      store = store.split('\\t')
      try:
        if not store[6].startswith('東濃信用金庫'): continue
      except IndexError:
        continue
