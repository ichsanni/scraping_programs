...
class RecycleHardoff(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/recycle/hardoff'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        for row in rows:
            try:
                ...
                self.content.append(self.get_data("REDACTED URL"+ path))
                self.save_data()
...
_store['address'] = soup_2.find('th', text='所在地').find_next_sibling('td').text
        maps = soup_2.find('a', {'class': 'link1'}).get('href')
...
_store['address']
        ...
        _store['tel_no'] = soup_2.find('th', text='電話番号').find_next_sibling('td').text
        _store['open_hours'] = soup_2.find('th', text='営業時間').find_next_sibling('td').text
        _store['gla'] = ''
