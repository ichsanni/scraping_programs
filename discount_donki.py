...
class DiscountDonki(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/discount/donki'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        data = {'word': '', 'biz_hours': '', 'area[2]': 'on', 'area[3]': 'on', 'area[4]': 'on', 'area[5]': 'on', 'area[6]': 'on', 'area[7]': 'on', 'area[8]': 'on', 'area[9]': 'on', 'area[1]': 'on', 'area[12]': 'on', 'zip01': '', 'zip02': '', 'business[1]': 'on', 'business[2]': 'on', 'business[5]': 'on', 'business[4]': 'on', 'business[10]': 'on', 'business[14]': 'on', 'op': 'search'}
        page =self.session.post(url, headers=headers, data=data)
        soup = BeautifulSoup(page.html.html, 'html.parser')
        domain = "REDACTED URL"
        content = list()
        store_lists = soup.find_all('div', {'class': 'shopList__store'})
        for store in store_lists:
            link = store.find('ul', 'shopList__linkBtnArea').find('a')['href']
            if 'uny.co.jp' in link:
                store_data = self.get_data_uny(link)
            else:
                store_data = self.get_data(domain + link)
            if store_data != '':
...
_store['address'] = soup.find('dt', text='住所').find_next_sibling('dd').text.replace('\t', '')
        try:
            maps_link = soup.find('a', {'id': 'congestion_time_link'})['href']
            try:
...
_store['address'])
            ...
            _store['lat'] = ''
            _store['lon'] = ''
        _store['tel_no'] = soup.find('span', 'telLink').text
...
_store['address'] = soup.find('th', text='所在地').find_next_sibling('td').text.replace('\t', '').replace('\n', '')
