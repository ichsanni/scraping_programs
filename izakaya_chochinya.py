...
class IzakayaChochinya(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_izakaya_chochinya.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        store_lists = soup.find_all('header', 'entry-header')
        for store in store_lists:
            if '鳥珍や' not in store.text: continue
...
_store['address'] = soup.find('th', text='住所').find_next_sibling('td').text
        ...
        _store['url_tenant'] = ''
        try: _store['営業時間'] = soup.find('th', text='営業時間').find_next_sibling('td').text # Open hours / Business Hours
        except AttributeError: _store['営業時間'] = ''
        try: maps_link = soup.select_one('*[href*="google.co.jp/maps"]')['href']
        except TypeError: maps_link = soup.select_one('*[src*="/maps"]')['src']
        coords = self.geo_loc_embed(maps_link)
        _store['lat'] = coords[0]
        _store['lon'] = coords[1]
        _store['tel_no'] = soup.find('th', text=re.compile('TEL')).find_next_sibling('td').text
        _store['gla'] = ''
        try: _store['定休日'] = soup.find('th', text='定休日').find_next_sibling('td').text # Regular holiday
