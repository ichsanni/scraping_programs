...
class SuperTaiyo(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/taiyo'.replace("/", "_")
        self.content = list()
...
_store['address'] = '〒' + soup.find('th', text='郵便番号').find_next_sibling('td').text + soup.find('th', text='住所').find_next_sibling('td').text
...
_store['address']
        ...
        _store['open_hours'] = soup.find('th', text='営業時間').find_next_sibling('td').text.split('※Google')[0].replace('\u3000', '').replace('\n', '')
        if re.search('休業中', _store['open_hours']):
            return
        else:
            pass
        _store['gla'] = ''
