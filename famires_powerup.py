...
class FamiresPowerup(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_famires_powerup.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        store_lists = soup.find_all('div', 'store_info')
        for store in store_lists:
            _store = dict()
            _store['store_name'] = store.find('h4').text
_store['address'] = re.sub('\s+', ' ', store.find('address').text.split('TEL')[0])
            _store['url_store'] = url
            _store['url_tenant'] = ''
            _store['営業時間'] = store.find('div', 'business_hours').text.replace('営業時間', '') # Open hours / Business Hours
            _store['lat'] = ''
            _store['lon'] = ''
            _store['tel_no'] = store.find('address').text.split('TEL')[1]
            _store['gla'] = ''
            _store['定休日'] = '' # Regular holiday
            _store['駐車場'] = '' # Parking lot Yes ( 有 ) , No ( 無 )
            _store['禁煙・喫煙'] = '' # [Non-smoking/Smoking] Yes ( 有 ) , No ( 無 )
            _store['取扱'] = '' # Handling
            _store['備考'] = re.sub('\s+', ' ', store.find('div', 'note').text) # Remarks
