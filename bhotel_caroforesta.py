...
class BhotelCaroforesta(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_bhotel_caroforesta.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        store_lists = soup.find_all('div', {'id':'left2'})
        print(len(store_lists))
        for store in store_lists:
            _store = dict()
            try: _store['store_name'] = store.find('span', {'id':'titl'}).text
            except AttributeError: continue
_store['address'] = re.findall('住所(.+)TEL', store.text, re.DOTALL)[0].replace('：', '').replace(':', '') # store.text.split('住所：')[1].split('TEL :')[0]
            _store['url_store'] = store.find('span', {'id':'titl'}).find('a')['href']
            _store['url_tenant'] = ''
            _store['営業時間'] = '' # Open hours / Business Hours
            _store['lat'] = ''
            _store['lon'] = ''
            _store['tel_no'] = re.findall('TEL(.+)', store.text, re.DOTALL)[0].replace('：', '').replace(':', '')
            _store['gla'] = ''
