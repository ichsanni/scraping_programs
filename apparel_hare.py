...
class ApparelHare(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/apparel/hare'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        try:
            ...
            return
_store['address'] = soup.find('th', text='住所').find_next_sibling('td').text.replace('Google Map', '').replace('\n', '').replace(' ', '')

        _store['lat'], _store['lon'] = '', ''
        _store['tel_no'] = soup.find('th', text='電話').find_next_sibling('td').text.replace('\n', '').replace(' ', '')
