...
class SweetsSweetsparadise(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/sweets/sweetsparadise'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find_all('li', 'shopinfo__detail')
        for store in store_lists:
            link = store.find_all('a')[-1]['href']
  ...
  """
        ...
        _store['store_name'] = soup.find('h1', 'archive__title').text.replace(' ', '').replace('\u3000', '')
...
_store['address']
        ...
        try:
            ...
            try:
                _store['tel_no'] = soup.find('dt', text='電話予約').find_next_sibling('dd').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', '')
            except AttributeError:
                _store['tel_no'] = ''
