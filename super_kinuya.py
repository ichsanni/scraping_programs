...
class SuperKinuya(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/kinuya'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find('section', {'id': 'page-shoplist'}).find_all('li')
        for store in store_lists:
            _store = dict()
            _store['url_store'] = url
            _store['store_name'] = store.find('h4').textaddress = store.find('dt', text='住所').find_next('dd')
            
_store['address'] = address.text

            _store['lat'], _store['lon'] = '', ''
            _store['tel_no'] = store.find('dt', text='TEL').find_next('dd').text
            _store['open_hours'] = store.find('dt', text='営業時間').find_next('dd').text
            _store['gla'] = ''
