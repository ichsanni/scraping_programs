...
class FamiresNikukome(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_famires_nikukome.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        store_lists = soup.select('table.c-list_indent')
        for store in store_lists:
            _store = dict()
            _store['store_name'] = store.find('th').text.strip()try: 
_store['address'] = store.find('p', text=re.compile(r'県|市|丁目|〒|区|村')).text.split('※')[0].strip()
            except:
                address = [x.text for x in store.find_all('p') if re.search('県|市|丁目|〒|区|村', x.text)][0]
                
_store['address'] = address.split('※')[0].strip()
