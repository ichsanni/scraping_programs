...
class BankSetoshin(CleaningData):
  def __init__(self, from_main=False):
    self.file_name = '_bank_setoshin.py'.replace('/', '_').replace('.py', '')
    ...
    start = time.time()
    # 35.270948236406205, 136.95456192441696
    url = "REDACTED URL"
    self.get_page(url)
    end = time.time()
  ...
  def get_page(self, url):
    ...
    for store in store_lists:
      store = store.split('\\t')
      try:
        if '瀬戸信用金庫' not in store[6]: continue
      except IndexError:
        continue
