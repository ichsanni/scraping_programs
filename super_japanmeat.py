...
class SuperJapanmeat(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/japanmeat'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        domain = "REDACTED URL"
        content = list()
        tables = soup.find_all('table', {'class': 'shop_tbl'})
        for table in tables:
            rows = table.find_all('tr')
            for row in rows:
                link = row.find('a').get('href')
                self.content.append(self.get_data(domain + link))
                self.save_data()
  ...
  """
        ...
        soup = BeautifulSoup(page.html.html, 'html.parser')
        _store = dict()
        details = soup.find('div', {'class': 'container_left'}).find_all('p')[0].text.split('\n')
        _store['url_store'] = url
        _store['store_name'] = soup.find('h2').text
_store['address'] = details[1].replace('\u3000', '').replace(' ', '')

        _store['lat'], _store['lon'] = '', ''

        _store['tel_no'] = details[2].replace('\u3000', '').split('FAX')[0].replace('  TEL.', '').replace('-', '')
        _store['open_hours'] = details[0].replace('\u3000', '')
        _store['gla'] = 'Null'
