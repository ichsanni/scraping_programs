...
class ChainIdToko(CleaningData):
  def __init__(self, from_main=False):
    self.file_name = '_gamecenter_dinos.py'.replace('/', '_').replace('.py', '')
  ...
  def get_page(self, url):
    ...
    store_lists = soup.find_all('article')
  ...
  def get_data(self, url):
    ...
    _store['store_name'] = soup.select_one('article header h1').text
_store['address'] = soup.find('span', text='住所').parent.find_next_sibling('dd').text.replace('google map', '').replace('\n', '')

    try: maps_link = soup.find('div', 'map_link').find('a')['href']
    except AttributeError: maps_link = "REDACTED URL"+ 
_store['address']

    _store['lat'], _store['lon'] = '', ''

    _store['tel_no'] = re.sub('\s+', ' ', soup.find('span', text='TEL').parent.find_next_sibling('dd').text)
