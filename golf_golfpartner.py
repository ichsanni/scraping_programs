...
class GolfGolfpartner(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/golf/golfpartner'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        print(len(store_json['store']))
        for store in store_json['store']:
            if int(store['prefcode']) < 48:
                _store = dict()
                link = domain + store['storecode']
                _store['url_store'] = link
                _store['store_name'] = store['name']
_store['address'] = '〒' + store['zipcode'] + store['address'].replace('\n', '')
                _store['lat'], _store['lon'] = '',''
                _store['tel_no'] = store['tel'].replace('-', '')
                page = self.session.get(link)
                soup = BeautifulSoup(page.html.html, 'html.parser')
                try:
                    _store['open_hours'] = soup.find_all('dl')[1].find('dd').text.split('※')[0].replace('\t', '').replace('\r', '').replace('\n', '').replace('\xa0', '').replace('\u3000', '').replace('<br>', '')
                except IndexError:
                    continue
                _store['gla'] = ''
