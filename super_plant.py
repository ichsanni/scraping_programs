...
class SuperPlant(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/plant'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        page = self.session.get(url)
        soup = BeautifulSoup(page.html.html, 'html.parser')
        details = soup.find('div', {'class': 'shop_info'}).find_all('dd')
        _store = dict()
        _store['url_store'] = url
        _store['store_name'] = soup.find_all('h1')[1].text
_store['address'] = details[0].text.replace('\n', '')
        try:
            maps = soup.find_all('div', {'class': 'chirashi_area'})[-1].find('a').get('href')
        except IndexError:
