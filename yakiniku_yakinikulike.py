...
class YakinikuYakinikulike(CleaningData):
  def __init__(self, from_main=False):
    ...
    if True:
      if from_main:
        self.df_clean.to_csv('D:/dags/csv/_yakiniku_yakinikulike.csv', index=False)
      else:
        # ======== UPLOAD KE BUCKET >>>>>>>>>
        client = storage.Client()
        bucket = client.get_bucket('scrapingteam')
        bucket.blob('/ichsan/_yakiniku_yakinikulike.csv').upload_from_string(
  ...
  def save_data(self):
    ...
    df.to_csv('D:/dags/csv/_yakiniku_yakinikulike.csv', index=False)
  ...
  def get_page(self, url, from_main):
    ...
    store_lists = soup.find_all('div', 'btn_box_YL')
  ...
  def get_data(self, url):
    ...
    _store['store_name'] = soup.find('h2', 'title_page_YK').text
_store['address'] = soup.find('h3', text='住所').find_next_sibling('p').text.split('TEL')[0].replace('\n',
                                                                                                       '').replace('\t',
                                                                                                                   '').replace(
    ...
    try:
      _store['tel_no'] = soup.find('h3', text='住所').find_next_sibling('p').text.split('TEL')[1].replace(':',
                                                                                                        '').replace(
        '\n', '').replace('\t', '').replace('\r', '').replace('\xa0', ' ').replace(' ', '')
    except IndexError:
      _store['tel_no'] = ''

    _store['open_hours'] = soup.find('h3', text='営業時間').find_next_sibling('p').text.replace('\n', '').replace('\t',
                                                                                                              '').replace(
