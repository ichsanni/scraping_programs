...
class IzakayaOstrea(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_izakaya_ostrea.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        store_lists = soup.find_all('div', 'yoyaku_box')
    ...
    def get_data(self, url):
        ...
        soup = BeautifulSoup(page.html.html, 'html.parser')
        _store = dict()
        _store['store_name'] = soup.find('div', 'shop_txt1').text
_store['address'] = ' '.join([x.strip() for x in soup.find('div', 'shop_txt2', text='住　　所')\
            .find_next_sibling('div').text.split() if '分' not in x])
        _store['url_store'] = url
        _store['url_tenant'] = ''
        _store['営業時間'] = soup.find('div', 'shop_txt2', text='営業時間').find_next_sibling('div').text.strip() # Open hours / Business Hours
        maps_link = soup.find('div', 'shop_googlemap').find('iframe')['src']
        coords = self.gmaps_embed_coords(self.session, maps_link)
        _store['lat'] = coords[0]
        _store['lon'] = coords[1]
        _store['tel_no'] = soup.find('div', 'shop_txt2', text='電話番号').find_next_sibling('div').text.strip()
        _store['gla'] = ''
