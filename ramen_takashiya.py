...
class RamenTakashiya(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_ramen_takashiya.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        for cat in -:
            ...
            for store in store_lists:
                if 'たかし屋' not in store.find('dt').text: continue
                _store = dict()
                _store['store_name'] = store.find('dt', text=re.compile('たかし屋')).text
_store['address'] = store.find('dd', 'address').text
                _store['url_store'] = url
                _store['url_tenant'] = ''
                _store['営業時間'] = store.find('dd', 'times').text.split('TEL')[0]  # Open hours / Business Hours
