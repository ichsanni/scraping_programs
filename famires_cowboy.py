...
class FamiresCowboy(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/famires/cowboy'.replace("/", "_")
        self.content = list()
        ...
        while True:
            self.seed = randint(110000000, 119999999)
...
_store['address'] = store['postal_code'] + ' ' + store['address_name'].replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', ' ')
...
_store['address']
            ...
            try:
                open_hours = [x['value'] for x in store['details'][0]['texts'] if x['label'] == '窓口営業時間'][0].lower().replace('br', '').replace('<', '').replace('>', '')
