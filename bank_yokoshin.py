...
class BankYokoshin(CleaningData):
  def __init__(self, from_main=False):
    self.file_name = '_bank_yokoshin.py'.replace('/', '_').replace('.py', '')
    ...
    start = time.time()
    # 35.508305779418755, 139.6770077974397
    url = "REDACTED URL"
    self.get_page(url)
    end = time.time()
  ...
  def get_page(self, url):
    ...
    for store in store_lists:
      store = store.split('\\t')
      try:
        if not store[6].startswith('横浜信用金庫'): continue
      except IndexError:
        continue
