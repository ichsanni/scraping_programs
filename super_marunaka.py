...
class SuperMarunaka(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/marunaka'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        domain = "REDACTED URL"
        store_lists = soup.find_all('li', {'class': 'result-item'})
  ...
  """
        ...
        page = self.session.get(url)
        soup = BeautifulSoup(page.html.html, 'html.parser')
        details = soup.find('div', {'class': 'basic-info'}).find_all('td')
        _store = dict()
        _store['url_store'] = url
        _store['store_name'] = soup.find('h1', {'class': 'tenpo-name'}).text
