...
class SportsMurasaki(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/sports/murasaki'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        domain = "REDACTED URL"
        content = list()
        for x in stores['data']['sorted_shops']:
            if x['name'] == 'ムラサキスポーツ':
                store_lists = x['shops']
                for store in store_lists:
                    _store = dict()
                    _store['url_store'] = domain + store['url']
                    _store['store_name'] = store['name']
_store['address'] = '〒' + store['zipcode'] + store['full_address']
                    if len(store['full_address']) == 0:
                        continue

                    _store['lat'], _store['lon'] = '', ''
                    _store['tel_no'] = store['tel']
                    _store['open_hours'] = store['open_hours']
                    _store['gla'] = ''
