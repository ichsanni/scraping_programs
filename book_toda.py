...
class BookToda(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/book/toda'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        domain = "REDACTED URL"
        content = list()
        store_lists = soup.find_all('td', {'class': 'btn'})
...
_store['address'] = soup.find('th', text='所在地').parent.text.replace('所在地', '')
        maps_link = "REDACTED URL"+ 
_store['address']

        _store['lat'], _store['lon'] = '', ''

        _store['tel_no'] = soup.find('th', text='電話番号').parent.text.replace('電話番号', '').replace('-', '')
        _store['open_hours'] = soup.find('th', text='営業時間').parent.text.replace('営業時間', '')
        _store['gla'] = ''
