...
class IzakayaHorumonyakikouei(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_izakaya_horumonyakikouei.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        store_lists = soup.find_all('div', 'cc-m-hgrid-column')
        for store in store_lists:
            if 'last' in store['class']: continue
            link = "REDACTED URL"+ store.select_one('a[data-title="詳しく見る＞"]')['href']
    ...
    def get_data(self, url):
        ...
        soup = BeautifulSoup(page.html.html, 'html.parser')
        _store = dict()
        _store['store_name'] = soup.find('center').text.strip()
_store['address'] = [x.text.replace("所在地", '')[1:] for x in soup.find_all('p') if "所在地" in x.text][0] # kadang pake : kadang pake ：
        _store['url_store'] = url
        _store['url_tenant'] = ''
        _store['営業時間'] = [x.text.replace("営業時間", '')[1:] for x in soup.find_all('p') if "営業時間" in x.text][0] # Open hours / Business Hours
        maps_link = soup.select_one('iframe[src*="maps"]')['src']
        try: coords = self.gmaps_embed_coords(self.session, maps_link)
        except TypeError: coords = ('', '')
        _store['lat'] = coords[0]
        _store['lon'] = coords[1]
        _store['tel_no'] = [x.text.replace("電話番号", '')[1:] for x in soup.find_all('p') if "電話番号" in x.text][0]
        _store['gla'] = ''
