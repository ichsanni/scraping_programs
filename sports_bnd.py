...
class SportsBnd(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/sports/bnd'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find('div', 'footerNav_list-shop').find_all('a')
        for store in store_lists:
            try:
                if 'shop_detail' not in store['href']:
                    continue
            except KeyError:
                continue
