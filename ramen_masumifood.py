...
class RamenMasumi(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_ramen_masumi.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        - = soup.find_all('div', 'store_list')
        for cat in -:
            store_lists = cat.find_all('dl')
            for store in store_lists:
                if '増田家' not in store.find('dt').text: continue
                _store = dict()
                _store['store_name'] = store.find('dt', text=re.compile('増田家')).text
_store['address'] = store.find('dd', 'address').text
                _store['url_store'] = url
                _store['url_tenant'] = ''
                _store['営業時間'] = store.find('dd', 'times').text.split('定休日')[0]  # Open hours / Business Hours
                maps_link = store.find('dd', 'btn').find('a')['href']
                coords = self.geo_loc_embed(maps_link)
                _store['lat'] = coords[0]
                _store['lon'] = coords[1]
                _store['tel_no'] = store.find('dd', 'times').text.split('TEL')[1]
                _store['gla'] = ''
                _store['定休日'] = store.find('dd', 'times').text.split('定休日')[1].split('TEL')[0]  # Regular holiday
