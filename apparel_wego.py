...
class ApparelWego(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/apparel/wego'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        areas = soup.find_all('div', 'swiper-wrapper')[1].find_all('a')[:-1]
        for area in areas:
            ...
            store_lists = area_soup.find_all('div', 'c-shop__info')
            for store in store_lists:
                _store = dict()
                _store['url_store'] = area_link
                _store['store_name'] = store.find('h2').text
_store['address'] = store.find('p', 'c-shop__adress').text

                _store['lat'], _store['lon'] = '', ''
                _store['tel_no'] = store.find('p', 'c-shop__tel').text.split('/')[-1].strip()
                _store['open_hours'] = store.find('p', 'c-shop__time').text.split('/')[-1].strip()
                _store['gla'] = ''
