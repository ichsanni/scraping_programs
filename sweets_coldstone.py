...
class SweetsColdstone(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/sweets/coldstone'.replace("/", "_")
        self.content = list()
        self.asession = None
  ...
  """
        ...
        content = list()
        areas = soup.find_all('ul', 'loacl_store')
        loop = asyncio.new_event_loop()

        for area in areas:
            store_lists = area.find_all('a')
            for store in store_lists:
                self.content.append(loop.run_until_complete(self.get_data(store['href'])))
                self.save_data()
  ...
  """
        ...
        _store['store_name'] = soup.find('div', 'storeInfo').find('h1').text
