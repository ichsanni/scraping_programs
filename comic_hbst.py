...
class ComicHbst(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/comic/hbst'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find('section', {'id': 'shoplist'}).find_all('a')
        for store in store_lists:
            if 'hbst.net/shop/' not in store['href']:
                continue
            self.content.append(self.get_data(store['href'], store.text))
            self.save_data()
