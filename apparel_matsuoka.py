...
class ApparelMatsuoka(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/apparel/matsuoka'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        domain = "REDACTED URL"
        content = list()
        store_lists = soup.find('div', {'id': 'contents2_right'}).find_all('a')
        for store in store_lists:
            if 'しの' in store.text or 'ウトレット' in store.text:
