...
class BankBanshin(CleaningData):
  def __init__(self, from_main=False):
    self.file_name = '_bank_banshin.py'.replace('/', '_').replace('.py', '')
    ...
    start = time.time()
    # 34.86010750880235, 134.5526364397501
    url = "REDACTED URL"
  ...
  def get_page(self, url):
    ...
    for store in store_lists:
      store = store.split('\\t')
      try:
        if not store[6].startswith('播州信用金庫'): continue
      except IndexError:
        continue
