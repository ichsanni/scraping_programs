...
class KadenYodobashi(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/kaden/yodobashi'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        domain = "REDACTED URL"
        content = list()
        store_lists = soup.find_all('div', {'class': 'container'})
        for store in store_lists:
            try:
                name = store.find('h2').text
                if 'ヨドバシ' in name:
                    link = domain + store.find('h2').find('a')['href']
                    addr = store.find('div', 'storeInfo_address').text
                    hours = store.find('div', 'storeInfo_saleTime').text
                    phone = store.find('div', 'storeInfo_tel').text
                    self.content.append(self.get_data(link, name, addr, hours, phone))
                    self.save_data()
                    print(len(self.content))
            except AttributeError:
                continue

    def get_data(self, url, name, addr, hours, phone):
        """
...
_store['address'] = addr
        ...
        _store['tel_no'] = phone.replace('\n', '')
        _store['open_hours'] = hours
        _store['gla'] = ''
