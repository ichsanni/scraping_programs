...
class SuperAngelfamily(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_super_angelfamily.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        # tahun udah 2023, tapi struktur web masih pake table
        # kayak website tahun < 2005 😭
        store_lists = soup.find_all('a', text=re.compile('周辺地図'))
    ...
    def get_data(self, url):
        ...
        soup = BeautifulSoup(page.html.html, 'html.parser')
        _store = dict()
        _store['store_name'] = soup.find('title').text
_store['address'] = soup.find('td', text=re.compile('住.+所')).find_next_sibling('td').text
        _store['url_store'] = url
        _store['url_tenant'] = ''
        _store['営業時間'] = soup.find('td', text=re.compile('営業時間')).find_next_sibling('td').text # Open hours / Business Hours
        maps_link = soup.find('iframe')['src'].replace('&output=embed', '')
        coords = self.geo_loc_embed(maps_link)
        _store['lat'] = coords[0]
        _store['lon'] = coords[1]
        tel_no = soup.find('td', text=re.compile('電.+話')).find_next_sibling('td').text
