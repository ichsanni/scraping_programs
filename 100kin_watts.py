...
class 100kinWatts(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/100kin/watts'.replace("/", "_")
        self.content = list()
        self.session = HTMLSession()

        self.start = time.time()
        self.urls = ["REDACTED URL" 'https://www.watts-jp.com/shop/?brandname%5B%5D=brand02&s=']
...
_store['address'] = soup.find('dt', text='住所').find_next('dd').text.replace('GoogleMapで確認する', '').replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', '')
