...
class SuperTachiya(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/tachiya'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find_all('div', {'class': 'item'})
        for store in store_lists:
            _store = dict()
            _store['url_store'] = url
            _store['store_name'] = store.find('p', 'item-title').text
_store['address'] = store.find('span', text='所在地').find_next('span').text
            try:
...
_store['address']
            ...
            _store['tel_no'] = store.find('span', text='TEL').find_next('a').text
            _store['open_hours'] = store.find('span', text='営業時間').find_next('p').text
            _store['gla'] = ''
