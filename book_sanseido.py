...
class BookSanseido(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/book/sanseido'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        domain = "REDACTED URL"
        content = list()
        areas = soup.find_all('table', {'class': 'shoplist'})
        for area in areas:
            store_lists = area.find_all('tr')
            for store in store_lists:
                try:
                    if 'http' in store.find('a')['href']:
                        continue
  ...
  """
        ...
        _store['store_name'] = soup.find('h1', 'lvl1').text
_store['address'] = soup.find('th', text='所在地').find_next_sibling('td').text.replace('\u3000', ' ').replace('\n', '')

        _store['lat'], _store['lon'] = '', ''
        try:
            _store['tel_no'] = soup.find('th', text='電話番号').find_next_sibling('td').find('a').text.replace('-', '')
        except AttributeError:
            _store['tel_no'] = ''
        try:
            _store['open_hours'] = soup.find('th', text='営業時間').find_next_sibling('td').text.split('\n')[0].replace('\u3000', ' ')
        except AttributeError:
            _store['open_hours'] = ''
        _store['gla'] = ''
