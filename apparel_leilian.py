...
class ApparelLeilian(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/apparel/leilian'.replace("/", "_")
        self.content = list()
        self.session = HTMLSession()
        self.content = list()
        self.areas_visited = list()
  ...
  """
        ...
        for area in areas:
            link = domain + area['href']
            if link in self.areas_visited:
                continue
            else:
                self.get_data(link)
                self.areas_visited.append(link)
  ...
  """
        ...
        soup = BeautifulSoup(page.html.html, 'html.parser')
        store_lists = soup.find_all('tr', {'class': 'region'})
        for store in store_lists:
            _store = dict()
            _store['url_store'] = url
            _store['store_name'] = store.find('td', {'class': 'store'}).textdetails = store.find('td', {'class': 'address'})
            
_store['address'] = details.contents[0]

            _store['lat'], _store['lon'] = '', ''
            _store['tel_no'] = details.find('a')['href'].replace('-', '').replace('tel:', '')
            _store['open_hours'] = ''
            _store['gla'] = ''
