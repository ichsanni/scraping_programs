...
class BhotelHiltonhotels(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_bhotel_hiltonhotels.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        store_lists = soup.select('div#gBodyFootNational a')
...
_store['address'] = soup.find('span', {'itemprop':'adress'}).text.strip()
        _store['url_store'] = url
        _store['url_tenant'] = ''
        _store['営業時間'] = '' # Open hours / Business Hours
        try:
            coords = self.session.get(page.url + '/access', headers=headers).html.find("div.hotelMapMarkerData")[0].attrs
            _store['lat'] = coords['marker-latitude']
            _store['lon'] = coords['marker-longitude']
        except:
            try:
                coords = self.session.get(page.url + '/access', headers=headers).html.find("div.hotelMapMarkerData")[
                    0].attrs
                _store['lat'] = coords['data-map-marker-latitude']
                _store['lon'] = coords['data-map-marker-longitude']
            except:
                raise
                _store['lat'] = ''
                _store['lon'] = ''

        try: _store['tel_no'] = soup.find('span', {'itemprop':'telephone'}).text.strip()
        except AttributeError: _store['tel_no'] = soup.find('div', {'itemprop':'telephone'}).text.strip()
        _store['gla'] = ''
