...
class CarUpgarage(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/car/upgarage'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find_all('div', {'class': 'shopbox'})
  ...
  """
        ...
        try:
            _store['store_name'] = soup.find('h1', 'hd1').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', '')
...
_store['address'] = soup.find('th', text='住所').find_next('td').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', '')
        ...
        try:
            _store['tel_no'] = soup.find('th', text='アップガレージ').find_next('td').find('a').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', '')
        except AttributeError:
