...
class HandcraftShimojima(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/handcraft/shimojima'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        domain = "REDACTED URL"
        content = list()
        store_lists = soup.find_all('div', {'class': 'name'})
  ...
  """
        ...
        page =self.session.get(url, headers=headers)
        soup = BeautifulSoup(page.html.html, 'html.parser')
        details = soup.find('table', 'store_info').find_all('td')
        _store = dict()
...
_store['address']
        ...
        _store['tel_no'] = details[1].text.split('：')[1].replace('FAX', '').replace('-', '')
