...
class BurgerVvdiner(CleaningData):
  def __init__(self, from_main=False):
    ...
    if True:
      if from_main:
        self.df_clean.to_csv('D:/dags/csv/_burger_vvdiner.csv', index=False)
      else:
        # ======== UPLOAD KE BUCKET >>>>>>>>>
        client = storage.Client()
        bucket = client.get_bucket('scrapingteam')
        bucket.blob('/ichsan/_burger_vvdiner.csv').upload_from_string(
  ...
  def save_data(self):
    if self.from_main:
      ...
      df.to_csv('D:/dags/csv/_burger_vvdiner.csv', index=False)
  ...
  def get_page(self, url):
    ...
    store_lists = soup.find_all('ul', 'shop-info')[-1].find_all('li')
...
_store['address'] = soup.find('dt', text='ADDRESS').find_next_sibling('dd').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', ' ')
    ...
    _store['open_hours'] = soup.find('dt', text='OP/CL').find_next_sibling('dd').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', ' ')
