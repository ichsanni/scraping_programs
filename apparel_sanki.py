...
class ApparelSanki(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/apparel/sanki'.replace("/", "_")
        self.content = list()
    ...
    """
        ...
        domain = "REDACTED URL"
        store_lists = soup.find_all('li', {'class': 'cf'})
        for store in store_lists:
            link = domain + store.find('a')['href']
            self.content.append(self.get_data(link))
            self.save_data()
            print(len(self.content))
        next_button = soup.find('ul', {'class': 'pager'}).find_all('li')
        if '次へ' in next_button[-1].text:
            next_page = domain + next_button[-1].find('a')['href']
...
_store['address']
        ...
        _store['open_hours'] = details[3].text
        _store['gla'] = ''
