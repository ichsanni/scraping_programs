...
class IzakayaSchmatz(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_izakaya_schmatz.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        store_lists = soup.find_all('div', 'feature-row__text')
        for store in store_lists:
            _store = dict()
            _store['store_name'] = store.find('h2', 'h3').text.strip()      # nama kelasnya h3, why;_;
_store['address'] = [x.get_text(separator='\n').split('\n')[0] for x in store.find('div', 'rte').find_all('p')
                                 if re.search('〒|県|市|町|丁目|区', x.text)][0].strip()
            _store['url_store'] = url
            _store['url_tenant'] = ''
            _store['営業時間'] = '' # Open hours / Business Hours
            maps_link = store.find('a', {'aria-label':'地図'})['href']
            coords = self.gmaps_embed_coords(self.session, maps_link)
            _store['lat'] = coords[0]
            _store['lon'] = coords[1]
            _store['tel_no'] = store.find('div', 'rte').text.replace(
_store['address'], '').strip()
            _store['gla'] = ''
