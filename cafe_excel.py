...
class CafeExcel(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/cafe/excel'.replace("/", "_")
  ...
  """
        ...
        if self.asession == None: self.asession = AsyncHTMLSessionFixed()
        ...
        for pref in home_json['data']:
            ...
            for area in area_json['data']:
                ...
                for point in point_json['items']:
                    if 'エクセル' in point['name'] and point['key'] not in self.pg_code:
                        link = domain + point['key']
                        self.content.append(await self.get_data(link, point['latitude'], point['longitude']))
                        self.save_data()
