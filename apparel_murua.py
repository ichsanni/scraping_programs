...
class ApparelMurua(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/apparel/murua'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        shop_tab = ['shop-tab1', 'shop-tab2']
        for tab in shop_tab:
            store_lists = soup.find('div', {'id': tab}).find_all('div', 'shop_data')
            for store in store_lists:
                _store = dict()
                _store['url_store'] = url
                _store['store_name'] = store.find('div', 'name').text
_store['address'] = store.find('div', 'info').text.replace('\n', '').replace('\t', '').replace('\r', '')
