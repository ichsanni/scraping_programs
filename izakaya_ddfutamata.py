...
class IzakayaDdfutamata(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_izakaya_ddfutamata.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        store_lists = soup.select('ul.locationsList01 > li')
    ...
    def get_data(self, url):
        ...
        soup = BeautifulSoup(page.html.html, 'html.parser')
        _store = dict()
        _store['store_name'] = soup.find('dt', text='店名').find_next_sibling('dd').text.strip().split('\n')[0]
_store['address'] = soup.find('dt', text='住所 / 地図').find_next_sibling('dd').text
        _store['url_store'] = url
        _store['url_tenant'] = ''
        _store['営業時間'] = soup.find('dt', text='営業時間').find_next_sibling(
            'dd').text.split('※')[0].replace('-', '')  # Open hours / Business Hours
        maps_link = soup.select_one('iframe[src*="maps/embed"]')['src']
        coords = self.geo_loc_embed(maps_link)
        _store['lat'] = coords[0]
        _store['lon'] = coords[1]
        _store['tel_no'] = soup.find('dt', text='電話番号').find_next_sibling('dd').text.strip().split('\n')[0]
        _store['gla'] = ''
        _store['定休日'] = soup.find('dt', text='定休日').find_next_sibling('dd').text.replace('\n',
                                                                                               '').split('※')[0] # Regular holiday
        _store['駐車場'] = ''  # Parking lot Yes ( 有 ) , No ( 無 )
        _store['禁煙・喫煙'] = ''  # [Non-smoking/Smoking] Yes ( 有 ) , No ( 無 )
        try:
            _store['取扱'] = '[総座席数]' + soup.find('dt', text='総座席数').find_next_sibling('dd').text.replace('\n',
                                                                                                                  '')  # Handling
