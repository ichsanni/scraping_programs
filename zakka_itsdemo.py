...
class ZakkaItsdemo(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/zakka/itsdemo'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        store_lists = soup.find_all('div', {'class': 'txt_shop-name'})
        ...
        try:
            next_button = soup.find('li', {'class': 'search-result__paging__last'}).find('a')['href']
  ...
  """
        ...
        page =self.session.get(url, headers=headers)
        soup = BeautifulSoup(page.html.html, 'html.parser')
        details = soup.find('div', {'class': 'txt_address'}).find('div').text.split('\n')
        _store = dict()
        _store['url_store'] = url
        _store['store_name'] = soup.find('h2').text.replace('\n', '').replace(' ', '').replace('\u3000', '')
_store['address'] = details[1] + details[2] + details[4].replace('\u3000', '')
