...
class IzakayaShinobuya(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/izakaya/shinobuya'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        store_lists = soup.find_all('li', 's_result_item')
        for store in store_lists:
            if "REDACTED URL" 'black').find('a')['href']: continue
            link = "REDACTED URL"+ store.find('div', 'black').find('a')['href']
  ...
  """
        ...
        _store['store_name'] = soup.find('th', text='店名').find_next_sibling('td').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', ' ')
