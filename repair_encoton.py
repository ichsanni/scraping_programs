...
class RepairEncoton(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/repair/encoton'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find_all('tr', 'hidden-xs')
  ...
  """
        ...
        _store['store_name'] = soup.find_all('h2', 'title-ribbon')[-1].text.strip()
_store['address'] = soup.find('th', text='所在地').find_next_sibling('td').text.strip()

        _store['lat'], _store['lon'] = '', ''
        _store['tel_no'] = soup.find('th', text=re.compile('電話番号')).find_next_sibling('td').text.strip()
        _store['open_hours'] = soup.find('th', text='営業時間').find_next_sibling('td').text.strip()
        _store['gla'] = ''
