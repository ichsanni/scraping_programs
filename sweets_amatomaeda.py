...
class SweetsAmatomaeda(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_sweets_amatomaeda.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        store_lists = soup.select('img[alt*="tenpo"]')
    ...
    def get_data(self, url):
        ...
        soup = BeautifulSoup(page.html.html, 'html.parser')
        _store = dict()
        _store['store_name'] = soup.find('div', {'id':'title'}).text.replace('のご案内', '').strip('◆')details = soup.find('div', {'id':'title'}).parent.get_text(separator='\n').split('\n')
        
_store['address'] = [x for x in details if '市' in x or '県' in x or '区' in x or '丁目' in x][0]
        _store['url_store'] = url
        _store['url_tenant'] = ''
        _store['営業時間'] = '' # Open hours / Business Hours
        maps_link = soup.find('a', text='地図で見る')['href']
