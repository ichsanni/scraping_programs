...
class SuperMatsuyasuper(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/matsuyasuper'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        _store['store_name'] = soup.find('h2').textaddress = soup.find('th', text='住所').find_next('td').text.split('TEL：')
        
_store['address'] = address[0].replace('\n', '').replace('\r', '')

        _store['lat'], _store['lon'] = '', ''
        _store['tel_no'] = address[1]
