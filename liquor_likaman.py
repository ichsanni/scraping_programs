...
class LiquorLikaman(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/liquor/likaman'.replace("/", "_")
        self.content = list()
...
_store['address'] = soup.find('th', text='住所').find_next('td').text.replace('\u3000', '').replace('\n', '').replace('\t', '').replace('\r', '')
        try:
