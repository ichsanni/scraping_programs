...
class BankHiroshin(CleaningData):
  def __init__(self, from_main=False):
    self.file_name = '_bank_hiroshin.py'.replace('/', '_').replace('.py', '')
    ...
    start = time.time()
    # 34.38243060673291, 132.45425322624527
    url = "REDACTED URL"
  ...
  def get_page(self, url):
    ...
    for store in store_lists:
      store = store.split('\\t')
      try:
        if not store[6].startswith('広島信用金庫'): continue
      except IndexError:
        continue
