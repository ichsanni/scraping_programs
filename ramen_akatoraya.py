...
class RamenAkatoraya(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_ramen_akatoraya.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        for cat in -:
            ...
            for store in store_lists:
                if '赤虎屋' not in store.find('dt').text: continue
                _store = dict()
                _store['store_name'] = store.find('dt', text=re.compile('赤虎屋')).text
