...
class CafeHayama(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/cafe/hayama'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        domain = "REDACTED URL"
        content = list()
        store_lists = soup.find_all('blockquote')
        for store in store_lists:
            store = store.parent.parent.select('div[data-testid="richTextElement"]')
            _store = dict()
            _store['url_store'] = url
            _store['store_name'] = store[0].text
_store['address'] = store[1].text
...
_store['address']
            ...
            _store['tel_no'] = store[-2].text
            _store['open_hours'] = store[-1].text
            _store['gla'] = ''
