import asyncio
import random
# from vpngate_l2tp.vpn import L2TP_VPNGate
from fake_useragent import UserAgent
import datetime, time
from bs4 import BeautifulSoup
from requests_html import HTMLSession, AsyncHTMLSession
import pyppeteer
import pandas as pd
import re
from scraping.scraping import CleaningData
try:
    from google.cloud import storage
except ModuleNotFoundError:
    pass

class AsyncHTMLSessionFixed(AsyncHTMLSession):
  """
  pip3 install websockets==6.0 --force-reinstall
  """

  def __init__(self, **kwargs):
    super(AsyncHTMLSessionFixed, self).__init__(**kwargs)
    self.__browser_args = kwargs.get("browser_args", ["--no-sandbox"])

  @property
  async def browser(self):
    if not hasattr(self, "_browser"):
      self._browser = await pyppeteer.launch(ignoreHTTPSErrors=not (self.verify), headless=True, handleSIGINT=False,
                                             handleSIGTERM=False, handleSIGHUP=False, args=self.__browser_args)

    return self._browser


class DrugSeijo(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/drug/seijo'.replace("/", "_")
        self.content = list()
        self.session = HTMLSession()
        self.asession = None
        self.PROXIES = {"REDACTED URL" 'REDACTED URL}
        self.url = "REDACTED URL"
        self.start = time.time()
        # self.vpn = L2TP_VPNGate()
        self.get_page(self.url)
        if True:
            self.end = time.time()
            
        x = pd.DataFrame(self.content)
        if len(x) == 0:
            raise ValueError('Empty df')
        
        x['lat'] = pd.to_numeric(x['lat'], errors='coerce')
        x['lon'] = pd.to_numeric(x['lon'], errors='coerce')
        x.loc[x[(x['lat'] < 20) | (x['lat'] > 50)].index, 'lat'] = pd.NA
        x.loc[x[(x['lat'] < 20) | (x['lat'] > 50)].index, 'lon'] = pd.NA
        x.loc[x[(x['lon'] < 121) | (x['lon'] > 154)].index, 'lat'] = pd.NA
        x.loc[x[(x['lon'] < 121) | (x['lon'] > 154)].index, 'lon'] = pd.NA
        x['url_tenant'] = None
        try:
            self.df_clean = self.clean_data(x)
        except:
            raise
        if True:
            if from_main:
                self.df_clean.to_csv(f'D:/dags/csv/{self.file_name}.csv', index=False)
            else:
                client = storage.Client()
                bucket = client.get_bucket('scrapingteam')
                bucket.blob(f'/ichsan/{self.file_name}.csv').upload_from_string(self.df_clean.to_csv(index=False), 'text/csv')

    def __str__(self) -> str:
        return str(self.df_clean.to_html(index=False, render_links=True, max_cols=25))

    def get_page(self, url):
        """
  Visit the link given from the gsheet,
  see if there's data there.
  If yes, scrape immediately.
  If no, call self.get_data(self.url) and visit individual pages.
  """
        headers = {'user-agent': UserAgent().random.strip(), 'cookie': 'wovn_selected_lang=ja'}
        page = self.session.get(url, headers=headers, proxies=self.PROXIES)
        soup = BeautifulSoup(page.html.html, 'html.parser')
        domain = "REDACTED URL"
        content = list()
        loop = asyncio.new_event_loop()
        store_lists = soup.find_all('li', {'class': 'shop_info'})
        for store in store_lists:
            link = domain + store.find('a')['href']
            self.content.append(loop.run_until_complete(self.get_data(link)))
            self.save_data()
            print(len(self.content))
            time.sleep(2)

    async def get_data(self, url):
        """
  Visit individual page,
  see if you can scrape map latitude and longitude.
  If no, visit map individuually by calling self.get_map_data(self.url)
  """
        print(url)
        headers = {'user-agent': UserAgent().random.strip(), 'cookie': 'wovn_selected_lang=ja'} # V1_store_history=100107%2F002448%2F100091%2F100297%2F100292; sn=01; V1_AIGENT_ID=C62BC7D538288E1D438DA0CF9E7133E7793C5FF5FD47E4D0; WAPID=GYJwTLMkduSKRPxEKL1KCp4tr7DdJELvDXn; wap_last_event=showWidgetPage; wovn_uuid=fbp8z0497; V1_accessmode=pc; V1_COMM_005=626A67C6A7EC193C9E882AF113D10CB5C9C811B1EAB697EA; JSESSIONID=6C3081F5C525347782B78326C5574E9D;
        if not self.asession: self.asession = AsyncHTMLSessionFixed()
        while True:
            try:
                page = await self.asession.get(url, headers=headers, proxies=self.PROXIES)
                await page.html.arender(timeout=0)
                soup = BeautifulSoup(page.html.html, 'html.parser')
                details = soup.find('ul', {'class': 'shop-detail__list'}).find_all('span', {'class': 'text'})
                break
            except AttributeError:
                # while True:
                #     try:
                #         self.vpn.connect('JP')
                #         break
                #     except RuntimeError: print('error encountered, repeating last action.')
                time.sleep(25)
        _store = dict()
        _store['url_store'] = url
        _store['store_name'] = soup.find('span', {'class': 'line'}).text
_store['address'] = details[1].text.replace('\n', '').replace('\r', '').replace('\t', '')
        _store['lat'], _store['lon'] = '',''
        _store['tel_no'] = details[2].text.replace('-', '').replace('\r', '').replace('\t', '').replace('\n', '')
        _store['open_hours'] = details[4].text.replace('\r', '').replace('\t', '').replace('\n', '')
        _store['gla'] = ''
        _store['scrape_date'] = datetime.date.today().strftime('%m/%d/%Y')
        # print(_store)
        return _store

    def get_map_data(self, url):
        headers = {'referer': "REDACTED URL"Not A;Brand";v="99", "Microsoft Edge";v="92"', 'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4515.159 Safari/537.36 Edg/92.0.902.84'}
        page =self.session.get(url, headers=headers, )    # proxies=self.PROXIES)
        return page

    def save_data(self):
        if self.from_main:
            df = pd.DataFrame(self.content)
            df = df.reindex(columns=['store_name', '-', '-', '-', '-', '-', '-', '-', 'address', 'url_store', 'url_tenant', 'open_hours', 'lat', 'lon', 'tel_no', 'gla', 'scrape_date'])
            df.to_csv(f'D:/dags/csv/{self.file_name}.csv', index=False)
if __name__ == '__main__':
    DrugSeijo(True)
