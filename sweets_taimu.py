...
class SweetsTaimu(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/sweets/taimu'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        visited_link = list()
        areas = soup.find('div', {'id': 'shop'}).find_all('div')
        for area in areas:
            try:
                link = domain + area.find('a')['href']
            except TypeError:
                continue
            rmv_id = re.sub('#\\w*', '', link)
            print(rmv_id)
            if rmv_id in visited_link:
                continue
            visited_link.append(rmv_id)
            area_page = self.session.get(rmv_id)
            area_soup = BeautifulSoup(area_page.html.html, 'html.parser')
            store_name = area_soup.find_all('p', 'shop_name')
            store_lists = area_soup.find_all('table')
            for store in zip(store_name, store_lists):
                _store = dict()
                _store['url_store'] = rmv_id
                _store['store_name'] = store[0].text.replace('\t', '')
_store['address'] = store[1].find('td', text='住所').find_next_sibling('td').text.replace('\t', '').replace('\u3000', '')
...
_store['address']
                ...
                try:
                    _store['tel_no'] = store[1].find('td', text='電話').find_next_sibling('td').text.replace('\t', '').replace('\u3000', '').replace('無', '').replace('-','')
                except AttributeError:
                    _store['tel_no'] = ''
                _store['open_hours'] = store[1].find('td', text='営業時間').find_next_sibling('td').text.replace('\t', '').replace('\u3000', '').replace('\n', '').replace('\xa0', '')
                _store['gla'] = ''
