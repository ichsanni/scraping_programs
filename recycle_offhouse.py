...
class RecycleOffhouse(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/recycle/offhouse'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        store_lists = soup.find('ul', {'class': 'store_list2'}).find_all('a')
        for store in store_lists:
            ...
            time.sleep(0.3)
        try:
            next_button = "REDACTED URL"+ soup.find('a', {'class': 'nextpostslink'})['href'].replace('./', '')
