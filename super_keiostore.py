...
class SuperKeiostore(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/keiostore'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        domain = "REDACTED URL"
        content = list()
        store_lists = soup.find_all('td', {'class': 'storeName'})
  ...
  """
        ...
        try:
            ...
            return
_store['address'] = details[0].find('td').text.replace('\u3000', '').replace('\n', '').replace('\r', '')
        _store['lat'], _store['lon'] = '', ''

        _store['tel_no'] = details[1].find('td').text.replace('-', '').replace('\n', '').replace('\r', '')
        _store['open_hours'] = details[3].find('td').text.replace('\n', '').replace('\r', '')
        _store['gla'] = 'Null'
