...
class SuitTokyoshirt(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/suit/tokyoshirt'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find_all('div', 'block-store-list--store-description')
        for store in store_lists:
          _store = dict()
          _store['url_store'] = "REDACTED URL"+ store.find('div', 'block-store-list--store-name').find('a')['href']
          _store['store_name'] = store.find('div', 'block-store-list--store-name').text.replace('\n', '').replace('\r', '').replace('\t', '')
_store['address'] = store.find('div', 'block-store-list--store-address').text.replace('\n', '').replace('\r', '').replace('\t', '')
...
_store['address']
          ...
          _store['tel_no'] = store.find('div', 'block-store-list--store-tel').text.replace('\n', '').replace('\r', '').replace('\t', '')
          _store['open_hours'] = store.find('div', 'block-store-list--store-sales-info').text.replace('\n', '').replace('\r', '').replace('\t', '')
          _store['gla'] = ''
