...
class BankShinkumi(CleaningData):
  def __init__(self, from_main=False):
    self.file_name = '_bank_shinkumi.py'.replace('/', '_').replace('.py', '')
    ...
    start = time.time()
    coord = [[34.69828, 133.21345], [40.60729, 140.85994]]
    for c in coord:
      url = f"REDACTED URL"\
            f"cid%3Dshinkumi%26opt%3Dshinkumi%26pos%3D1%26cnt%3D1000%26enc%3DEUC%26" \
            f"lat%3D{c[0]}%26lon%3D{c[1]}%26latlon%3D%26jkn%3D%26rad%3D5000000%26knsu%3D1000%26exkid%3D%26" \
            f"hour%3D1%26cust%3D%26exarea%3D%26polycol%3D&zdccnt=3&enc=EUC&encodeflg=0"
      self.get_page(url)
    end = time.time()
