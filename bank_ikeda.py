...
class BankIkeda(CleaningData):
  def __init__(self, from_main=False):
    self.file_name = '_bank_ikeda.py'.replace('/', '_').replace('.py', '')
  ...
  def get_page(self, url):
    ...
    for store in store_lists:
      ...
      if store[0] not in self.visited_stores and store[8]:
        ...
        link = f"REDACTED URL"
        self.content.append(self.get_data(link, store[6], store[9], store[10]))
        print(len(self.content))
        self.save_data()


  def get_data(self, url, store_name, zipcode, addr):
    '''
...
_store['address'] = zipcode + addr.split('<BR>')[0].split('<br>')[0]


        _store['lat'], _store['lon'] = '', ''

        try: _store['tel_no'] = soup.find('td', text=re.compile('\(0\d+\)\d+')).text.replace('\n', '').replace('\t', '').replace('\r', '').replace(' ', '')
