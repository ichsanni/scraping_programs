...
class UsedcarNextage(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/usedcar/nextage'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        visited_url = list()
        areas = soup.find('ul', 'kyoten').find_all('a')
        for area in areas:
            ...
            store_lists = area_soup.find_all('div', 'img_box')
  ...
  """
        ...
        try:
            _store['store_name'] = soup.find('caption').text.replace('\u3000', '')
        except AttributeError:
            return None
_store['address'] = soup.find('th', text='住所').find_next_sibling('td').text
        try:
            maps = soup.find('iframe', 'indoorviewbox')['data-src']
        except TypeError:

            _store['lat'], _store['lon'] = '', ''
        _store['tel_no'] = soup.find('th', text='電話番号').find_next_sibling('td').text
        _store['open_hours'] = ''.join([x.text for x in soup.find('ul', 'shopopen').find_all('li')[1:]])
        _store['gla'] = ''
