...
class IzakayaBuaiso(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_izakaya_buaiso.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        store_lists = soup.find_all('a', 'detail_link')
    ...
    def get_data(self, url):
        ...
        soup = BeautifulSoup(page.html.html, 'html.parser')
        _store = dict()
        _store['store_name'] = soup.find('h1', 'shop_name').text.split('ぶあいそ')[-1].split('博多')[-1].split('（')[0].strip()
_store['address'] = soup.find('p', 'shop_address').text.strip()
        ...
        _store['lat'] = coords[0]
        _store['lon'] = coords[1]
        _store['tel_no'] = soup.find('p', 'shop_tel').text.strip()
        _store['gla'] = ''
