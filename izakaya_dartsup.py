...
class IzakayaDartsup(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_izakaya_dartsup.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        store_lists = soup.find('ul', 'shops').find_all('a')
    ...
    def get_data(self, url):
        ...
        soup = BeautifulSoup(page.html.html, 'html.parser')
        _store = dict()
        _store['store_name'] = soup.find('h2', 'mh').text.strip()
_store['address'] = soup.find('dt', text='住所').find_next_sibling('dd').text
        if 
_store['address'] == '':
            
_store['address'] = soup.find('dt', text='住所').find_next_sibling('dt').text
        _store['url_store'] = url
        _store['url_tenant'] = ''
        _store['営業時間'] = "".join([x.text for x in soup.find('dt', text=re.compile('営業時間')).find_next_siblings('dd')]) # Open hours / Business Hours
        _store['lat'] = page.content.decode('utf8').split('"lat": ')[-1].split(',')[0]
        _store['lon'] = page.content.decode('utf8').split('"lng": ')[-1].split(',')[0]
        _store['tel_no'] = soup.find('dt', text=re.compile('電話')).find_next_sibling('dd').text
        _store['gla'] = ''
        _store['定休日'] = soup.find('dt', text='定休日').find_next_sibling('dd').text # Regular holiday
