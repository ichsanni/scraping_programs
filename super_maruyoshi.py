...
class SuperMaruyoshi(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/maruyoshi'.replace("/", "_")
        self.content = list()
        self.asession = None
  ...
  """
        ...
        content = list()
        store_lists = soup.find('div', {'class': 'shopList'}).find_all('a')
        loop = asyncio.new_event_loop()
        

        for store in store_lists:
            link = store.get('href')
            self.content.append(loop.run_until_complete(self.get_data(self.url + link)))
            self.save_data()
...
_store['address']
        ...
        try:
            _store['tel_no'] = details[1].text.replace('（', '').replace('）', '').replace('-', '')
        except IndexError:
            _store['tel_no'] = ''
        try:
            _store['open_hours'] = details[2].text
        except IndexError:
            _store['open_hours'] = ''
        _store['gla'] = 'Null'
