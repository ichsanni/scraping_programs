...
class BankHamamatsuiwata(CleaningData):
  def __init__(self, from_main=False):
    self.file_name = '_bank_hamamatsuiwata.py'.replace('/', '_').replace('.py', '')
    ...
    start = time.time()
    # 34.82153696506486, 137.8017301262571
    url = "REDACTED URL"
    self.get_page(url)
    end = time.time()
  ...
  def get_page(self, url):
    ...
    for store in store_lists:
      store = store.split('\\t')
      try:
        if not store[6].startswith('浜松磐田信用金庫'): continue
      except IndexError:
        continue
