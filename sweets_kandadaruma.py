...
class SweetsKandadaruma(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/sweets/kandadaruma'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find_all('section', 'shopBox')
        for store in store_lists:
            _store = dict()
            _store['url_store'] = url
            _store['store_name'] = store.find('h2').text.replace('たいやき神田達磨', '').strip()
_store['address'] = store.find('dt', text='所在地').find_next_sibling('dd').text
