...
class SweetsSadamatu(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_sweets_sadamatu.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        store_lists = soup.find_all('div', 'shopbox')
        for store in store_lists:
            if '定松' not in store.text: continue
            _store = dict()
            store_name = store.find(text=re.compile('「.+」'))
            _store['store_name'] = re.findall('「(.+)」', store_name.text)[0]maps_link = store.select_one("a[href*='/maps/']")
            
_store['address'] = maps_link.text
            _store['url_store'] = url
            _store['url_tenant'] = ''
            _store['営業時間'] = '' # Open hours / Business Hours
            # coords = self.geo_loc_embed(maps_link['href'])
            # NOTE: script gmaps dibawah akan error, karena di web gmapsnya
            # tidak ada marker merah yang menunjukkan lokasi tepatnya.
            _store['lat'] = ''
            _store['lon'] = ''
            _store['tel_no'] = store.find(text=re.compile('TEL')).text.replace('TEL ', '')
            _store['gla'] = ''
            _store['定休日'] = '' # Regular holiday
            _store['駐車場'] = '' # Parking lot Yes ( 有 ) , No ( 無 )
            _store['禁煙・喫煙'] = '' # [Non-smoking/Smoking] Yes ( 有 ) , No ( 無 )
            _store['取扱'] = '' # Handling
            _store['備考'] = re.findall('」(.+)', store_name.parent.text)[0] # Remarks
