...
class SuperMammys(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/mammys'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        href = list()
        store_lists = soup.find_all('area')
        for store in store_lists:
            if store['href'] not in href:
                link = domain + store['href']
                self.content.append(self.get_data(link))
                self.save_data()
                print(len(self.content))
                time.sleep(1)
                href.append(store['href'])
  ...
  """
        ...
        page =self.session.get(url, headers=headers)
        soup = BeautifulSoup(page.html.html, 'html.parser')
        details = soup.find('div', {'id': 'StoreName'})
        _store = dict()
        _store['url_store'] = url
        _store['store_name'] = details.find('h2').text
_store['address'] = details.find('th', text='所在地').find_next_sibling('td').text.replace('\u3000', '')
        maps_link = "REDACTED URL"+ 
_store['address']

        _store['lat'], _store['lon'] = '', ''

        _store['tel_no'] = details.find('th', text='TEL').find_next_sibling('td').text
        _store['open_hours'] = details.find('th', text='営業時間').find_next_sibling('td').text
        _store['gla'] = ''
