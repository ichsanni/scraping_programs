...
class PcSmalabo(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/pc/smalabo'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find('div', 'Box').find_all('a')
  ...
  """
        ...
        while True:
            try:
                page = self.session.get(url, headers=headers, proxies=self.PROXIES)
                soup = BeautifulSoup(page.html.html, 'html.parser')
                soup.find('div', text='店舗住所').find_next_sibling('div')
                break
            except Exception as e:
                print('unexpected error:', e.args)
                # while True:
                #     try: self.vpn.connect()
                #     except: pass
                time.sleep(5)
        _store = dict()
        _store['url_store'] = url
        _store['store_name'] = soup.find('h1').texttry: 
_store['address'] = soup.find('div', text='店舗住所').find_next_sibling('div').text
        except AttributeError: 
_store['address'] = soup.find('div', text=re.compile('店舗住所')).find_next_sibling('div').text
        _store['lat'], _store['lon'] = '',''
        _store['tel_no'] = soup.find('div', text='電話番号').find_next_sibling('div').text
        _store['open_hours'] = soup.find('div', text='営業時間').find_next_sibling('div').text
        _store['gla'] = ''
