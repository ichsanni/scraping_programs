...
class ConveniYamazaki(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/conveni/yamazaki'.replace("/", "_")
        self.content = list()
        ...
        latlonlist = [[31.919452370455307, 130.77563856932127], [34.859800192143815, 136.43051728180663], [40.760387258126805, 141.00713525192373]]
        for x in latlonlist:
  ...
  """
        ...
        while True:
            try:
                ...
                time.sleep(5)
        new_page = page.content.decode('euc-jp').split('=')[1].replace(';', '').replace("'", '')
        domain = "REDACTED URL"
        store_lists = new_page.replace('\\t', ',').split('\\n')[1:]
        for store in store_lists:
            details = store.split(',')
            if len(details) > 9 and details[0] not in self.visited_stores:
                self.visited_stores.append(details[0])
                _store = dict()
                _store['url_store'] = domain + details[0]
                _store['store_name'] = details[6]
_store['address'] = '〒' + details[8] + details[7]
