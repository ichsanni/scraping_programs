...
class BankSkshinkin(CleaningData):
  def __init__(self, from_main=False):
    self.file_name = '_bank_skshinkin.py'.replace('/', '_').replace('.py', '')
    ...
    start = time.time()
    # 34.76671859925793, 138.01446249556736
    url = "REDACTED URL"
    self.get_page(url)
    end = time.time()
  ...
  def get_page(self, url):
    ...
    for store in store_lists:
      store = store.split('\\t')
      try:
        if not store[6].startswith('島田掛川信用金庫'): continue
      except IndexError:
        continue
