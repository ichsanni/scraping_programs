...
class SuperMarukyu(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/marukyu'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find_all('td', {'class': 'logo-title'})
        for store in store_lists:
            if 'マルキュウ' in store.find('a').text:
...
_store['address'] = details[0].text.replace('\r', '').replace('\n', '').replace('\u3000', '')
        location = re.findall('\\d{2,3}\\.\\d{3,}', soup.find('iframe').get('src'))
        if location:
