...
class SuperRgenki(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/rgenki'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        domain = "REDACTED URL"
        content = list()
        store_lists = soup.find('h2').parent.find_next('div').find_all('li')
  ...
  """
        ...
        _store['store_name'] = soup.find('span', 'ttl').text
_store['address'] = soup.find('th', text=re.compile('住所')).find_next('td').text.replace('\n', '').replace(' ', '')
        maps_link = "REDACTED URL"+ 
_store['address']

        _store['lat'], _store['lon'] = '', ''

        _store['tel_no'] = soup.find('th', text=re.compile('TEL')).find_next('td').text
        _store['open_hours'] = soup.find('th', text=re.compile('営業時間')).find_next('td').text
        _store['gla'] = ''
