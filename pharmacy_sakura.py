...
class PharmacySakura(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/pharmacy/sakura'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find_all('div', {'class': 'search_result_item'})
...
_store['address'] = soup.find('th', text='住所').find_next_sibling('td').text.replace('\n', '').replace('\u3000', '')
        ...
        _store['tel_no'] = soup.find('th', text='電話').find_next_sibling('td').text
