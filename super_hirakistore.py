...
class SuperHirakistore(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/hirakistore'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find_all('section', {'class': 'col'})
        for store in store_lists:
            if '本部' not in store.find('h2').text:
                details = store.find('p').text.split('TEL')
                _store = dict()
...
_store['address'] = details[0]
                ...
                _store['tel_no'] = details[1].split('FAX')[0]
                _store['open_hours'] = store.find('p', 'eigyou').text.replace('営業時間', '')
                _store['gla'] = ''
