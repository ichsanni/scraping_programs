...
class SweetsLaduree(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_sweets_laduree.py'.replace('/', '_').replace('.py', '')
    ...
    def get_data(self, url):
        ...
        while True:
            try:
                ...
                page = self.session.get(url, headers=headers)
                soup = BeautifulSoup(page.html.html, 'html.parser')
                details = soup.find('p', 'mapLink').parent
                break
            except AttributeError:
                # websitenya rada sensitif kalo sering kescrape.
                print('website tidak terload dengan baik, retrying.')
                if error_counter > 5: raise
                error_counter += 1
                time.sleep(3)
                self.session.close()
                self.session = HTMLSession()
        _store = dict()
        _store['store_name'] = details.select_one('.shoplist-title').find('span').text
_store['address'] = details.find('p', 'txt2').text.split('TEL')[0].split('営業時間')[0]
        _store['url_store'] = url
        _store['url_tenant'] = ''
        _store['営業時間'] = ' '.join([x.replace('営業時間', '') for x in details.find('p', 'txt2').get_text(separator='\n').split('\n')
                              if '営業時間' in x]).strip('：:') # Open hours / Business Hours
        maps_link = soup.find('p', 'mapLink').find('a')['href']
        coords = self.geo_loc_embed(maps_link)
        _store['lat'] = coords[0]
        _store['lon'] = coords[1]
        try:
            _store['tel_no'] = [x.replace('TEL', '') for x in details.find('p', 'txt2').get_text(separator='\n').split('\n')
                              if 'TEL' in x][0].strip('：:')
