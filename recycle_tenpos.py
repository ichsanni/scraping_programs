...
class RecycleTenpos(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/recycle/tenpos'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        domain = "REDACTED URL"
        content = list()
        areas = soup.find_all('table', 'tenpo-t')
        for area in areas:
            store_lists = area.find_all('a')
            for store in store_lists:
                link = domain + store['href'] if 'http' not in store['href'] else store['href']
  ...
  """
        ...
        _store['store_name'] = soup.find('h1').text.split('（')[0]
_store['address'] = soup.find('th', text='住所').find_next_sibling('td').text.replace('\u3000', '')
        ...
        try: _store['open_hours'] = soup.find('th', text='営業時間').find_next_sibling('td').text.replace('\u3000', '').replace('\n', '')
