...
class BankKawashin(CleaningData):
  def __init__(self, from_main=False):
    self.file_name = '_bank_kawashin.py'.replace('/', '_').replace('.py', '')
    ...
    start = time.time()
    # 35.551781866866385, 139.6781983109329
    url = "REDACTED URL"
    self.get_page(url)
    end = time.time()
  ...
  def get_page(self, url):
    ...
    for store in store_lists:
      store = store.split('\\t')
      try:
        if not store[6].startswith('川崎信用金庫'): continue
      except IndexError:
        continue
