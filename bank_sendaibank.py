...
class BankSendaibank(CleaningData):
  def __init__(self, from_main=False):
    ...
    if True:
      if from_main:
        self.df_clean.to_csv('D:/dags/csv/_bank_sendaibank.csv', index=False)
      else:
        # ======== UPLOAD KE BUCKET >>>>>>>>>
        client = storage.Client()
        bucket = client.get_bucket('scrapingteam')
        bucket.blob('/ichsan/_bank_sendaibank.csv').upload_from_string(
  ...
  def save_data(self):
    if self.from_main:
      ...
      df.to_csv('D:/dags/csv/_bank_sendaibank.csv', index=False)
  ...
  def get_page(self, url):
    ...
    for store in store_lists:
      ...
      is_atm = True if icons else False
      if not is_atm:
        if '店' not in store.find('a').text: continue
      _store = dict()
...
_store['address']
      ...
      try:
        _store['open_hours'] = store_soup.find('tr', text='平日').find_next('td').text
