...
class SuperNishitetsu(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/nishitetsu'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        domain = "REDACTED URL"
        content = list()
        store_lists = soup.find_all('ul', {'class': 'shop-box'})
        for store in store_lists:
            if 'あんくるふじや' not in store.find('a').find_previous_sibling('span').text:
                try:
...
_store['address'] = soup.find('dt', text=re.compile('住所')).find_next_sibling('dd').text
