...
class NailFastnail(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/nail/fastnail'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        _store['store_name'] = soup.find('div', 'pageSubTtl').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', ' ').strip()try:
            
_store['address'] = soup.find('a', 'mapBtn').parent.find_next_sibling('dd').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', ' ').strip()
...
_store['address'] = soup.find('dt', text=re.compile('住所')).find_next_sibling('dd').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', ' ').strip()
        ...
        _store['tel_no'] = soup.find('dt', text='ご予約電話番号').find_next_sibling('dd').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', ' ').strip()
