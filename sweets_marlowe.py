...
class SweetsMarlowe(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/sweets/marlowe'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find_all('div', 'shop-list-item')
        for store in store_lists:
            store_name = store.find('h3').text
            address = store.find('dl', 'shop-info-address').text
            tel_no = store.find('dl', 'shop-info-tel').text
            self.content.append(self.get_data(store.find_all('a')[-1]['href'], store_name, address, tel_no))
            self.save_data()
