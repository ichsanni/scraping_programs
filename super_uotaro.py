...
class SuperUotaro(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_super_uotaro.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        store_lists = soup.find_all('span', 'subList_store')
        for store in store_lists:
            link = f"REDACTED URL"
            if 'temporary' in link or 'laguna' in link: continue
...
_store['address'] = soup.find('div', 'h_map').text.split('TEL')[0].replace('／', '')
        _store['url_store'] = url
        _store['url_tenant'] = ''
        _store['営業時間'] = '' # Open hours / Business Hours
        maps_link = soup.find('div', 'h_map').select_one('a[href*="/maps/"]')['href']
        coords = self.geo_loc_embed(maps_link)
        _store['lat'] = coords[0]
        _store['lon'] = coords[1]
        _store['tel_no'] = soup.find('div', 'h_map').text.split('TEL')[1].split('／')[0].split('\n')[0]
        _store['gla'] = ''
