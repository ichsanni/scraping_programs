...
class CarrierBiglobe(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/carrier/biglobe'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        for store in store_lists['counters']:
            if store['endDate'] != '':
                continue
            _store = dict()
            _store['url_store'] = "REDACTED URL"
            _store['store_name'] = store['name']
_store['address'] = store['prefecture'] + store['address1'] + store['address2']
            maps_link = "REDACTED URL"+ 
_store['address']

            _store['lat'], _store['lon'] = '', ''
            _store['tel_no'] = store['tel']
            _store['open_hours'] = '午前 ' + store['time1'] + '～' + '午後 ' + store['time2']
            _store['gla'] = ''
