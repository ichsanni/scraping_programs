...
class SuperNishigaki(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/nishigaki'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        for store in store_lists:
            if '店' in store.text:
                details = store.parent.find_next_sibling('li')
                _store = dict()
                _store['url_store'] = url
                _store['store_name'] = store.text
_store['address'] = details.find('p', 'address').text
                maps_link = "REDACTED URL"+ 
_store['address']

                _store['lat'], _store['lon'] = '', ''
                _store['tel_no'] = details.find('dt', text='電話番号').find_next('dd').text
                _store['open_hours'] = details.find('dt', text='営業時間').find_next('dd').text
                _store['gla'] = ''
