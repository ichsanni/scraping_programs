...
class ApparelAnteprima(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/apparel/anteprima'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        _store['store_name'] = soup.find('section', {'id': 'stores_detail'}).find('h3').text
_store['address'] = soup.find('h4', text='住所').find_next_sibling('p').text
        maps_link = "REDACTED URL"+ 
_store['address']

        _store['lat'], _store['lon'] = '', ''
        _store['tel_no'] = soup.find('td', 'tel').text
        _store['open_hours'] = ''
        _store['gla'] = ''
