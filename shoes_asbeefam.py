...
class ShoesAsbeefam(CleaningData):

  def __init__(self, from_main=False):
    self.from_main = from_main

    self.file_name = '/shoes/asbeefam'.replace("/", "_")
    self.content = list()
...
"""
    ...
    domain = "REDACTED URL"
    content = list()
    areas = soup.find_all('table', {'class': 'table--bordered'})
    for area in areas:
...
"""
    ...
    _store['store_name'] = soup.find('h2', {'class': 'heading'}).text
_store['address'] = details[2].text.replace('\n', '').replace('\u3000', '').replace('MAPを見る', '')
...
_store['address']
    ...
    _store['open_hours'] = details[1].text.split('：')[1]
    _store['gla'] = ''
