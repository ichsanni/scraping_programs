...
class BikeparkingChurins(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_bikeparking_churins.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        areas = soup.find('div', 'manage_navi').find_all('a')
        for area in areas:
            # if "REDACTED URL"in area['href']: continue
            if 'm01_' not in area['href']: continue
            area_link = f"REDACTED URL"
            area_page = self.session.get(area_link, headers=headers)
            area_soup = BeautifulSoup(area_page.html.html, 'html.parser')

            store_lists = area_soup.find_all('div', 'manage_box')
            for store in store_lists:
                _store = dict()
                _store['store_name'] = store.find('h3').text
_store['address'] = store.find('th', text='所在地').find_next_sibling('td').text
                _store['url_store'] = area_link
                _store['url_tenant'] = ''
                _store['営業時間'] = store.find('th', text='利用時間').find_next_sibling('td').text # Open hours / Business Hours
                _store['lat'] = ''
                _store['lon'] = ''
                try: _store['tel_no'] = store.find('th', text='お問合せ先').find_next_sibling('td').text
                except AttributeError: _store['tel_no'] = ''
                _store['gla'] = ''
                _store['定休日'] = '' # Regular holiday
                try: _store['駐車場'] = store.find('th', text='収容台数').find_next_sibling('td').text # Parking lot Yes ( 有 ) , No ( 無 )
