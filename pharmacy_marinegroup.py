...
class PharmacyMarinegroup(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/pharmacy/marinegroup'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find_all('ul', 'pg-sub-storelist')
        for store in store_lists:
            store_url = store.find('a')['href']
            self.content.append(self.get_data(store_url))
            self.save_data()
  ...
  """
        ...
        _store['store_name'] = soup.find('h2').text.strip().replace('\u3000', ' ').replace('\n', '').replace('\t', '').replace('\r', '')
_store['address'] = soup.find('th', text='住所').find_next_sibling('td').text.strip().replace('\u3000', ' ').replace('\n', '').replace('\t', '').replace('\r', '')
...
_store['address']
        ...
        _store['tel_no'] = soup.find('th', text='電話番号').find_next_sibling('td').text.strip().replace('\u3000', ' ').replace('\n', '').replace('\t', '').replace('\r', '')
        _store['open_hours'] = soup.find('th', text='開局時間').find_next_sibling('td').text.strip().replace('\u3000', ' ').replace('\n', '').replace('\t', '').replace('\r', '').split('※')[0]
        _store['gla'] = ''
