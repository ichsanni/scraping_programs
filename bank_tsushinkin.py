...
class BankTsushinkin(CleaningData):
  def __init__(self, from_main=False):
    self.file_name = '_bank_tsushinkin.py'.replace('/', '_').replace('.py', '')
    ...
    start = time.time()
    # 34.73354741702782, 136.51257616858274
    url = "REDACTED URL"
  ...
  def get_page(self, url):
    ...
    for store in store_lists:
      store = store.split('\\t')
      try:
        if not store[6].startswith('津信用金庫'): continue
      except IndexError:
        continue
