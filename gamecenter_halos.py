...
class GamecenterHalos(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_gamecenter_halos.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        store_lists = soup.select('div.area-body > ul')
        for store in store_lists:
            _store = dict()
            _store['store_name'] = store.find('li', 'store-name').text.replace('\n', '').replace('\r', '').strip()
_store['address'] = store.find(text=re.compile('[県都府道町丁目区〒]')).text.strip()
            _store['url_store'] = url
            _store['url_tenant'] = ''
            _store['営業時間'] = store.find(text=re.compile('営業時間')).text.replace('営業時間：', '').strip() # Open hours / Business Hours
            _store['lat'] = ''
            _store['lon'] = ''
            ...
            _store['取扱'] = ', '.join([x.text.replace('\n', '').replace('\r', '').strip() for x in store.select('ul.store-item-list li')]) # Handling
            _store['備考'] = '' # Remarks
