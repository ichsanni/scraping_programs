...
class SuperMiyazakimatuno(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/miyazakimatuno'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        for store in store_lists:
            details = store.find_next_sibling('p').text.split('\n')
            _store = dict()
            _store['url_store'] = url
            _store['store_name'] = store.text
_store['address'] = ''.join(details[0:2]).replace(' ', '')
