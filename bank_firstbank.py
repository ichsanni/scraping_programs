...
class BankFirstbank(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/bank/firstbank'.replace("/", "_")
        self.content = list()
...
_store['address']
            ...
            _store['tel_no'] = ''
            try:
                open_hours = [x['value'] for x in store['details'][0]['texts'] if x['label'] == 'ATM営業時間'][0].lower().replace('br', '').replace('<', '').replace('>', '')
            except IndexError:
                open_hours = ''
