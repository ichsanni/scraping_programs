...
class TrunkSyunopit(CleaningData):
  def __init__(self, from_main=False):
    self.file_name = '/trunk/syunopit'.replace('/', '_')
    self.from_main = from_main
    # Taruh semua variabel yang ada di luar class/methods disini
    self.session = HTMLSession()
    self.content = list()
    self.visited_link = list()
  ...
  def get_page(self, url):
    ...
    area_lists = soup.select('div.area-nav a') # div.area-nav a # li.col-4 a
    for area in area_lists:
      ...
      store_lists = area_soup.select('ul.spot-list a')
      for store in store_lists:
        store_link = re.sub('\s', '', store['href'])
        if store_link in self.visited_link: continue
        self.visited_link.append(store_link)
