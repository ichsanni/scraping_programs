...
class IzakayaKamonfskabune(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_izakaya_kamonfskabune.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        store_lists = soup.find_all('td', 'wysiwyg_template_profile2_wraptext')
        for store in store_lists:
            _store = dict()
            _store['store_name'] = store.find('span', {'style':'font-size: 18pt;'}).text.strip()
_store['address'] = '〒' + store.text.split('〒')[-1].strip()
            _store['url_store'] = url
            _store['url_tenant'] = ''
            _store['営業時間'] = '' # Open hours / Business Hours
            _store['lat'] = ''
            _store['lon'] = ''
            _store['tel_no'] = store.text.split(_store['store_name'])[-1].split(
_store['address'])[0].strip()
            _store['gla'] = ''
