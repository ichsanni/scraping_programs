...
class WorkTamayura(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/work/tamayura'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        domain = "REDACTED URL"
        content = list()
        store_lists = soup.find_all('a', 'shopBox')
...
_store['address']
        ...
        _store['open_hours'] = soup.find('th', text='開店時間').find_next_sibling('td').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', '').replace(' ', '')
        _store['gla'] = ''
