...
class SuperPurecook(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/purecook'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        domain = "REDACTED URL"
        content = list()
        store_lists = soup.find_all('div', {'class': 'type008-area'})
...
_store['address'] = soup.find('td', text='所在地').find_next('td').text.replace('\u3000', '')
        maps_link = [x['href'] for x in soup.find_all('a', 'ad-banner-link') if 'goo.gl' in x['href']][0]

        _store['lat'], _store['lon'] = '', ''
        _store['tel_no'] = soup.find('td', text='電話番号').find_next('td').text
        _store['open_hours'] = soup.find('td', text='営業時間').find_next('td').text
        _store['gla'] = ''
