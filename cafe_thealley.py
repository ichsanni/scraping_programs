...
class CafeThealley(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/cafe/thealley'.replace("/", "_")
        self.content = list()
        self.asession = None
  ...
  """
        ...
        content = list()
        store_lists = soup.select('ul.store-list > li')
        for store in store_lists:
            _store = dict()
            _store['url_store'] = url
            _store['store_name'] = store.find('li', 'store_name').text
...
_store['address']
            ...
            try:
                _store['tel_no'] = store.find('p', 'phone').text.split('：')[1]
