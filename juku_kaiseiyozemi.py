...
class JukuKaiseiyozemi(CleaningData):
  def __init__(self, from_main=False):
    self.file_name = '/juku/kaiseiyozemi'.replace('/', '_')
  ...
  def get_page(self, url):
    ...
    store_lists = BeautifulSoup(page.html.html, 'lxml')

    for store in store_lists.find_all('marker'):
      data = self.get_data(store['data-sidelink'], store['lat'], store['lng'])
      if data:
...
_store['address'] = soup.find('th', text='住所').find_next_sibling('td').text.replace('\u3000', ' ')
    ...
    _store['open_hours'] = soup.find('th', text='開校時間').find_next_sibling('td').text.replace('\u3000', ' ')
