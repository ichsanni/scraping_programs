...
class FamiresLeetantancafe(CleaningData):
    def __init__(self, from_main=False):
        self.file_name = '_famires_leetantancafe.py'.replace('/', '_').replace('.py', '')
    ...
    def get_page(self, url):
        ...
        store_lists = soup.find('a', {'name':'leetantan'}).find_next_sibling('table').find_all('tr')
        for store in store_lists:
            _store = dict()
            _store['store_name'] = store.find('strong').text
_store['address'] = store.find('th').text.replace(_store['store_name'], '').split('【TEL】')[0]
            _store['url_store'] = url
            _store['url_tenant'] = ''
            _store['営業時間'] = '' # Open hours / Business Hours
            _store['lat'] = ''
            _store['lon'] = ''
            _store['tel_no'] = store.find('th').text.replace(_store['store_name'], '').split('【TEL】')[1]
            _store['gla'] = ''
