...
class BookAthine(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/book/athine'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        areas = soup.find('dl', class_='shop_list_top').find_all('dt')
        for area in areas:
            area_link = "REDACTED URL"+ area.find('a').get('href').replace('./', '')
            area_page =self.session.get(area_link, headers=headers)
            area_soup = BeautifulSoup(area_page.html.html, 'html.parser')
            store_lists = area_soup.find('table', 'shop_all').find_all('a')
            for store in store_lists:
                if 'アシーネ' not in store.text:
                    continue
...
_store['address'] = soup.find('th', text='住\u3000\u3000所').find_next_sibling('td').text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\u3000', ' ')
