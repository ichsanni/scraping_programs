...
class SweetsFlo(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/sweets/flo'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        areas = soup.find_all('table', 'shop_tab')
        for area in areas:
            ...
            for store in store_lists:
                ...
                _store['store_name'] = details[0].text.split('店')[0].replace('\n', '').replace('\t', '').replace('\r', '').replace('\xa0', '') + '店'
_store['address'] = details[2].text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\xa0', '')

                _store['lat'], _store['lon'] = '', ''
                _store['tel_no'] = details[3].text.replace('\n', '').replace('\t', '').replace('\r', '').replace('\xa0', '')
