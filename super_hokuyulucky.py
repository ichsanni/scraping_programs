...
class SuperHokuyulucky(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/hokuyulucky'.replace("/", "_")
        self.content = list()
  ...
  """
        ...
        content = list()
        store_lists = soup.find_all('a', {'class': 'btn-shop'})
  ...
  """
        ...
        _store['store_name'] = soup.find('h1', {'class': 'entry-title'}).text
_store['address'] = details[0].text.replace('\n', '').replace('  ', '').replace('\u3000', '')
        maps = soup.find('a', {'class': 'btn-warning'}).get('href')
