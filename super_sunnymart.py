...
class SuperSunnymart(CleaningData):

    def __init__(self, from_main=False):
        self.from_main = from_main
 
        self.file_name = '/super/sunnymart'.replace("/", "_")
        self.content = list()
        self.asession = None
  ...
  """
        ...
        content = list()
        store_lists = soup.find('div', {'class': 'shop'}).find_all('a')
  ...
  """
        ...
        soup = BeautifulSoup(page.html.html, 'html.parser')
        details = soup.find('div', {'id': 'ShopInfoAddress'}).find_all('dd')
        _store = dict()
        _store['url_store'] = url
        _store['store_name'] = soup.find('h1').text.replace('\u3000', '')
_store['address'] = details[0].text.replace('\n', '').replace('\u3000', '')
        maps = soup.find('div', {'id': 'ShopInfoMap'})
        location = re.findall('\\d{2,3}\\.\\d{3,}', str(maps))
        if location:
